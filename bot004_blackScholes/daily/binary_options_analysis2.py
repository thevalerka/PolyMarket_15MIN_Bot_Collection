...
# Per-minute statistics (minute 0-14)
pnl_by_minute_and_probability = {
    minute: {prob: {'total_pnl': 0, 'num_trades': 0, 'wins': 0, 'losses': 0} 
             for prob in probability_levels}
...
def get_minute_bucket(record_dt, period_start_dt):
    ...
    # Clamp to 0-14 range (in case of slight timing issues)
...
def process_file(file_path, previous_last_period):
    ...
    for i, period_key in enumerate(sorted_periods):
        ...
        # Collect all probability levels reached during this period, by minute
        levels_by_minute = defaultdict(set)
        for record in period_records:
            ...
            levels_by_minute[minute_bucket].add(rounded_prob)
        ...
        period_outcomes.append({
            'period': period_key,
            'levels_by_minute': levels_by_minute,
            'call_won': call_won,
    ...
    file_stats_by_minute = {
        minute: {prob: {'pnl': 0, 'trades': 0, 'wins': 0, 'losses': 0} 
                 for prob in probability_levels}
        for minute in range(15)
    }
    ...
    for outcome in period_outcomes:
        if outcome['call_won'] is not None:
            ...
            # For each minute and each probability level reached
            for minute_bucket, levels_in_minute in outcome['levels_by_minute'].items():
                for prob_level in probability_levels:
                    if prob_level in levels_in_minute:
                        cost = prob_level
                        
                        if outcome['call_won']:
                            profit = 1.0 - cost
                            file_stats[prob_level]['wins'] += 1
                            file_stats_by_minute[minute_bucket][prob_level]['wins'] += 1
                        else:
                            profit = -cost
                            file_stats[prob_level]['losses'] += 1
                            file_stats_by_minute[minute_bucket][prob_level]['losses'] += 1
                        
                        file_stats[prob_level]['pnl'] += profit
                        file_stats[prob_level]['trades'] += 1
                        
                        file_stats_by_minute[minute_bucket][prob_level]['pnl'] += profit
                        file_stats_by_minute[minute_bucket][prob_level]['trades'] += 1
    ...
    return {
        ...
        'file_stats_by_minute': file_stats_by_minute,
...
for file_path in file_paths:
    try:
        ...
        # Aggregate per-minute statistics
        for minute in range(15):
            for prob_level in probability_levels:
                pnl_by_minute_and_probability[minute][prob_level]['total_pnl'] += result['file_stats_by_minute'][minute][prob_level]['pnl']
                pnl_by_minute_and_probability[minute][prob_level]['num_trades'] += result['file_stats_by_minute'][minute][prob_level]['trades']
                pnl_by_minute_and_probability[minute][prob_level]['wins'] += result['file_stats_by_minute'][minute][prob_level]['wins']
                pnl_by_minute_and_probability[minute][prob_level]['losses'] += result['file_stats_by_minute'][minute][prob_level]['losses']
...
# Calculate win rates and averages for per-minute stats
for minute in range(15):
    for prob_level in probability_levels:
        data = pnl_by_minute_and_probability[minute][prob_level]
        if data['num_trades'] > 0:
            data['win_rate'] = data['wins'] / data['num_trades']
            data['avg_pnl_per_trade'] = data['total_pnl'] / data['num_trades']
        else:
...
# Find best probability level for each minute
best_by_minute = {}
for minute in range(15):
    minute_profitable = {p: data for p, data in pnl_by_minute_and_probability[minute].items() if data['num_trades'] > 0}
    if minute_profitable:
        best_by_minute[minute] = {
            'best_total_pnl': max(minute_profitable.keys(), key=lambda p: minute_profitable[p]['total_pnl']),
            'best_avg_pnl': max(minute_profitable.keys(), key=lambda p: minute_profitable[p]['avg_pnl_per_trade'])
        }

# Write detailed TXT results
...
with open(output_file, 'w') as f:
    ...
    f.write("Strategy: When option price reaches a probability level, buy $1\n")
    ...
    # Overall statistics
    f.write("OVERALL PNL BY PROBABILITY LEVEL (All Minutes Combined):\n")
    f.write("-" * 100 + "\n")
    ...
    if best_prob:
        f.write("BEST OVERALL PROBABILITY LEVELS:\n")
        f.write(f"  Highest Total PNL: {best_prob:.2f} (${pnl_by_probability[best_prob]['total_pnl']:.2f} from {pnl_by_probability[best_prob]['num_trades']} trades)\n")
        f.write(f"  Best Avg PNL/Trade: {best_avg_pnl_prob:.2f} (${pnl_by_probability[best_avg_pnl_prob]['avg_pnl_per_trade']:.4f} from {pnl_by_probability[best_avg_pnl_prob]['num_trades']} trades)\n")
        f.write(f"  Worst Total PNL: {worst_prob:.2f} (${pnl_by_probability[worst_prob]['total_pnl']:.2f})\n\n")
    
    f.write("=" * 100 + "\n\n")
    
    # Per-minute analysis
    f.write("BEST PROBABILITY LEVEL FOR EACH MINUTE:\n")
    f.write("-" * 100 + "\n")
    f.write(f"{'Minute':<10} {'Best (Total PNL)':<20} {'Total PNL':<15} {'Trades':<10} {'Best (Avg PNL)':<20} {'Avg PNL':<12}\n")
    f.write("-" * 100 + "\n")
    
    for minute in range(15):
        if minute in best_by_minute:
            best_total = best_by_minute[minute]['best_total_pnl']
            best_avg = best_by_minute[minute]['best_avg_pnl']
            f.write(f"{minute:<10} {best_total:<20.2f} ${pnl_by_minute_and_probability[minute][best_total]['total_pnl']:<14.2f} "
                   f"{pnl_by_minute_and_probability[minute][best_total]['num_trades']:<10} "
                   f"{best_avg:<20.2f} ${pnl_by_minute_and_probability[minute][best_avg]['avg_pnl_per_trade']:<11.4f}\n")
    
    f.write("\n" + "=" * 100 + "\n\n")
    
    # Detailed per-minute breakdown
    f.write("DETAILED PNL BY MINUTE AND PROBABILITY LEVEL:\n\n")
    
    for minute in range(15):
        f.write(f"\n--- MINUTE {minute} (seconds {minute*60}-{(minute+1)*60-1}) ---\n")
        f.write("-" * 100 + "\n")
        f.write(f"{'Prob':<8} {'Trades':<8} {'Wins':<8} {'Losses':<8} {'Win Rate':<10} {'Total PNL':<12} {'Avg PNL':<12}\n")
        f.write("-" * 100 + "\n")
        
        for prob in sorted(probability_levels):
            data = pnl_by_minute_and_probability[minute][prob]
            if data['num_trades'] > 0:
                f.write(f"{prob:<8.2f} {data['num_trades']:<8} {data['wins']:<8} {data['losses']:<8} "
                       f"{data['win_rate']:<10.2%} ${data['total_pnl']:<11.2f} ${data['avg_pnl_per_trade']:<11.4f}\n")
...
print(f"\nTXT results written to {output_file}")
...
json_output = {
    ...
    'overall_statistics': {},
    'best_overall': {
        'best_total_pnl_probability': float(best_prob) if best_prob else None,
        'best_avg_pnl_probability': float(best_avg_pnl_prob) if best_avg_pnl_prob else None,
        'worst_total_pnl_probability': float(worst_prob) if worst_prob else None
    },
    'by_minute_statistics': {},
    'best_by_minute': {}
}
...
for prob in probability_levels:
    data = pnl_by_probability[prob]
    if data['num_trades'] > 0:
        json_output['overall_statistics'][f'{prob:.2f}'] = {
...
# Add per-minute statistics to JSON
for minute in range(15):
    json_output['by_minute_statistics'][f'minute_{minute}'] = {}
    for prob in probability_levels:
        data = pnl_by_minute_and_probability[minute][prob]
        if data['num_trades'] > 0:
            json_output['by_minute_statistics'][f'minute_{minute}'][f'{prob:.2f}'] = {
                'probability': float(prob),
                'num_trades': data['num_trades'],
                'wins': data['wins'],
                'losses': data['losses'],
...
# Add best by minute to JSON
for minute, best_data in best_by_minute.items():
    best_total = best_data['best_total_pnl']
    best_avg = best_data['best_avg_pnl']
    json_output['best_by_minute'][f'minute_{minute}'] = {
        'minute': minute,
        'best_total_pnl': {
            'probability': float(best_total),
            'total_pnl': pnl_by_minute_and_probability[minute][best_total]['total_pnl'],
            'num_trades': pnl_by_minute_and_probability[minute][best_total]['num_trades']
        },
        'best_avg_pnl': {
            'probability': float(best_avg),
            'avg_pnl_per_trade': pnl_by_minute_and_probability[minute][best_avg]['avg_pnl_per_trade'],
            'num_trades': pnl_by_minute_and_probability[minute][best_avg]['num_trades']
        }
    }

# Write JSON output
json_output_file = "binary_option_pnl_analysis.json"
...
if best_prob:
    print(f"\nOVERALL BEST PROBABILITY LEVEL (Total PNL): {best_prob:.2f}")
    ...
    print("BEST PROBABILITY BY MINUTE:")
    for minute in range(15):
        if minute in best_by_minute:
            best_total = best_by_minute[minute]['best_total_pnl']
            print(f"  Minute {minute}: {best_total:.2f} (${pnl_by_minute_and_probability[minute][best_total]['total_pnl']:.2f})")
else:
