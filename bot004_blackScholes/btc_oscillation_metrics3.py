...
class Enhanced15MinBinaryOptionCalculator:
    ...
    def calculate_oscillation_metrics(self, lookback_minutes=60):
        ...
        WITH RECENCY WEIGHTING for ChopIdx and Fractal
        Returns: dict with 4 key metrics measuring path complexity and oscillation intensity
        """
        ...
        # 2. CHOPPINESS INDEX WITH EXPONENTIAL RECENCY WEIGHTING
        # More weight to recent price movements
        # Calculate exponential weights (more recent = higher weight)
        time_indices = np.arange(len(prices))
        decay_factor = 0.95  # Higher = more emphasis on recent data
        weights = decay_factor ** (len(prices) - 1 - time_indices)
        weights = weights / np.sum(weights)  # Normalize
        
        # Weighted price movements
        movements = np.abs(np.diff(prices))
        weighted_movements = movements * weights[1:]  # Exclude first weight (no movement there)
        weighted_sum = np.sum(weighted_movements) * len(movements)  # Rescale
        ...
        # 3. ZERO-CROSSING RATE WITH RECENCY WEIGHTING
        # Recent direction changes weighted more heavily
        ...
        # 4. FRACTAL DIMENSION WITH RECENCY EMPHASIS (1.0 to 2.0)
        # Focus on most recent 30 minutes for fractal calculation
...
def test_oscillation_metrics():
    ...
    # 2. Choppiness Index (WEIGHTED - most important)
    ci = metrics['choppiness_index']
    if ci is not None:
        print(f"\n2️⃣  CHOPPINESS INDEX (RECENCY-WEIGHTED): {ci:.2f} ⭐")
    ...
    if zcr is not None:
        print(f"\n3️⃣  ZERO-CROSSING RATE (RECENCY-WEIGHTED): {zcr:.3f} changes/min")
        ...
        print(f"\n3️⃣  ZERO-CROSSING RATE: N/A (insufficient data)")
    
    # 4. Fractal Dimension (LAST 30 MIN - most important)
    fd = metrics['fractal_dimension']
    if fd is not None:
        print(f"\n4️⃣  FRACTAL DIMENSION (LAST 30 MIN): {fd:.3f} ⭐")
        ...
        print(f"\n4️⃣  FRACTAL DIMENSION: N/A (insufficient data)")
    ...
    if all(v is not None for v in metrics.values()):
        # Score the choppiness (0-4, higher = more choppy) - removed path_ratio
        chop_score = 0
        ...
        print(f"\n📊 KEY INDICATORS (recency-weighted):")
        print(f"   • ChopIdx: {ci:.1f} {'📈 CHOPPY' if ci > 61.8 else '📉 TRENDING' if ci < 38.2 else '➡️  MIXED'}")
        print(f"   • Fractal: {fd:.3f} {'🔴 COMPLEX' if fd > 1.6 else '🟢 SMOOTH' if fd < 1.3 else '🟡 NORMAL'}")
    else:
