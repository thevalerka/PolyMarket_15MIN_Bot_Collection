...
class BinaryOptionsPairTrader:
    ...
    def _save_live_data(self, pair_prices):
        """Save current pair prices to JSON for live dashboard"""
        now = datetime.now()
        ...
        # Always save all pairs in the same order
        for pair in self.pairs:
            pair_name = pair['name']
            
            if pair_name in pair_prices:
                ...
                live_data['pairs'].append({
                    ...
                    'exit_price': round(data['exit_price'], 6),
                    'spread': round(spread, 6),
                    'call_ask': round(data['call_ask'], 6),
                    'call_bid': round(data['call_bid'], 6),
                    'put_ask': round(data['put_ask'], 6),
                    'put_bid': round(data['put_bid'], 6),
                    'cached': False
                })
            else:
                # Pair missing - this shouldn't happen with caching, but just in case
                live_data['pairs'].append({
                    'name': pair_name,
                    'entry_price': 0,
                    'exit_price': 0,
    ...
    def _print_current_prices(self, pair_prices):
        ...
        if not pair_prices:
            ...
            for pair_name in sorted(pair_prices.keys()):
                ...
                spread = exit_val - entry
                spread_str = f"{spread:.6f}"
                
                exit_str = f"{exit_val:.6f}"
                call_bid_str = f"{data['call_bid']:.6f}"
                put_bid_str = f"{data['put_bid']:.6f}"
                ...
                print(f"{pair_name:<30} {entry:<18.6f} {exit_str:<18} {spread_str:<28} {data['call_ask']:<12.6f} {data['put_ask']:<12.6f} {call_bid_str:<12} {put_bid_str:<12}")
