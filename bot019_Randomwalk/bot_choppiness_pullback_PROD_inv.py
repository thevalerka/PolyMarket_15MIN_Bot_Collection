...
pm2 start bot_choppiness_pullback_PROD_inv.py --cron-restart="00 */6 * * *" --interpreter python3
"""
...
class ChoppinessPullbackBot:
    ...
    def verify_position_from_wallet(self):
        ...
        if wallet_call_positions != self.call_positions:
            if wallet_call_positions > self.call_positions:
                ...
                self.max_call_ask = 0.0
                logger.info(f"✅ CALL position filled: {self.call_positions} → {wallet_call_positions}")
            else:
        ...
        if wallet_put_positions != self.put_positions:
            if wallet_put_positions > self.put_positions:
                # Position filled! Reset max for next entry
                self.max_put_ask = 0.0
                logger.info(f"✅ PUT position filled: {self.put_positions} → {wallet_put_positions}")
            else:
    ...
    def calculate_current_pnl(self, call_ask: Optional[float], put_ask: Optional[float]) -> float:
        """
        Calculate current unrealized PNL for all positions
        ...
        Returns:
            Total PNL (positive or negative)
        """
        if not call_ask or not put_ask:
            return 0.0

        # Estimate average entry price (0.5 as fallback if we don't have last entry)
        call_avg_entry = self.last_call_entry_price if self.last_call_entry_price else 0.5
        put_avg_entry = self.last_put_entry_price if self.last_put_entry_price else 0.5

        # Calculate PNL for each side
        call_pnl = (call_ask - call_avg_entry) * self.call_positions * POSITION_SIZE
        put_pnl = (put_ask - put_avg_entry) * self.put_positions * POSITION_SIZE
        ...
        while True:
            try:
                ...
                if positions_are_even and not self.period_stopped_positive_pnl:
                    current_pnl = self.calculate_current_pnl(call_ask, put_ask)
                    logger.info(f"\n💰 EVEN POSITIONS CHECK: {self.call_positions}C / {self.put_positions}P")
