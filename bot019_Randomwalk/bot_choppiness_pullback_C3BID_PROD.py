...
pm2 start bot_choppiness_pullback_C3BID.py --cron-restart="00 */6 * * *" --interpreter python3
"""
...
MAX_POSITIONS_PER_SIDE_HARDCAP = 3  # Initial value, will be adapted
...
class ChoppinessPullbackBot:
    def __init__(self):
        ...
        self.trades_dir = f"{self.data_dir}/tradesC3BID"
        self.state_file = f"{self.data_dir}/stateC3BID.json"
        ...
        while True:
            try:
                ...
                if self.call_positions < self.current_max_positions and  self.call_positions < MAX_POSITIONS_PER_SIDE_HARDCAP:
                    ...
                    if self.call_positions < self.put_positions :
                        pullback_weighted = (PULLBACK_AMOUNT*choppiness/10)/2
                        if self.call_positions >= 3 :
                            pullback_weighted = (PULLBACK_AMOUNT*choppiness/10)/1.5
                    ...
                    if self.max_call_ask > 0 and call_ask and call_ask <= (self.max_call_ask - pullback_weighted):
                        ...
                        logger.info(f"   (Adaptive: pullback=${pullback_weighted}, max_pos={self.current_max_positions})")
