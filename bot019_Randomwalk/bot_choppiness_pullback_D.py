#!/usr/bin/env python3
"""
Strategy D - Adaptive Choppiness-Based Trading Bot
...
- Saves parameters to state and applies to next period
pm2 start bot_choppiness_pullback_D.py --cron-restart="00 */6 * * *" --interpreter python3
"""
...
# Initial Configuration (will be adapted based on previous period)
...
class ChoppinessPullbackBot:
    def __init__(self):
        ...
        # ADAPTIVE PARAMETERS - Will be updated based on previous period
        self.current_max_positions = MAX_POSITIONS_PER_SIDE
        self.current_pullback = PULLBACK_AMOUNT

        # Track retracement stats from previous period
        self.previous_call_retracements = 0
        self.previous_put_retracements = 0
        self.previous_avg_retracement = 0.0

        # Data persistence
        self.data_dir = "/home/ubuntu/013_2025_polymarket/bot019_Randomwalk"
        self.trades_dir = f"{self.data_dir}/tradesD"
        self.state_file = f"{self.data_dir}/stateD.json"
        ...
        logger.info(f"✓ STRATEGY D - ADAPTIVE PARAMETERS")
        ...
        logger.info(f"✓ CURRENT Max positions: {self.current_max_positions} per side")
        logger.info(f"✓ CURRENT Pullback base: ${self.current_pullback:.3f}")
        logger.info(f"✓ Adaptation: MAX_POS = min(call_ret, put_ret), PULLBACK = avg_ret - 0.01")
    ...
    def load_state(self):
        ...
        try:
            ...
            # Restore adaptive parameters from last period
            adaptive_params = state.get('adaptive_parameters', {})
            if adaptive_params:
                self.current_max_positions = adaptive_params.get('max_positions', MAX_POSITIONS_PER_SIDE)
                self.current_pullback = adaptive_params.get('pullback', PULLBACK_AMOUNT)
                self.previous_call_retracements = adaptive_params.get('call_retracements', 0)
                self.previous_put_retracements = adaptive_params.get('put_retracements', 0)
                self.previous_avg_retracement = adaptive_params.get('avg_retracement', 0.0)
                logger.info(f"📊 Loaded adaptive params: MAX_POS={self.current_max_positions}, PULLBACK=${self.current_pullback:.3f}")
    ...
    def update_adaptive_parameters(self, call_retracements: int, put_retracements: int,
                                   call_avg_width: float, put_avg_width: float):
        """Update adaptive parameters based on retracement analysis"""
        # Calculate new max positions: minimum of call and put retracements
        # Ensure at least 1 position per side
        new_max_positions = max(1, min(call_retracements, put_retracements))

        # Calculate new pullback: average of call and put avg widths, minus 0.01
        # Ensure pullback is at least 0.01
        combined_avg = (call_avg_width + put_avg_width) / 2 if (call_avg_width > 0 and put_avg_width > 0) else \
                       (call_avg_width if call_avg_width > 0 else put_avg_width if put_avg_width > 0 else 0.03)
        new_pullback = max(0.05, combined_avg - 0.01)

        logger.info("=" * 80)
        logger.info("🔧 ADAPTIVE PARAMETER UPDATE")
        logger.info(f"Previous period retracements: CALL={call_retracements}, PUT={put_retracements}")
        logger.info(f"Previous period avg widths: CALL=${call_avg_width:.3f}, PUT=${put_avg_width:.3f}")
        logger.info(f"Combined avg retracement: ${combined_avg:.3f}")
        logger.info("-" * 80)
        logger.info(f"OLD: MAX_POSITIONS={self.current_max_positions}, PULLBACK=${self.current_pullback:.3f}")
        logger.info(f"NEW: MAX_POSITIONS={new_max_positions}, PULLBACK=${new_pullback:.3f}")
        logger.info("=" * 80)

        # Update parameters
        self.current_max_positions = new_max_positions
        self.current_pullback = new_pullback
        self.previous_call_retracements = call_retracements
        self.previous_put_retracements = put_retracements
        self.previous_avg_retracement = combined_avg
    ...
    def save_state(self, market_data: Optional[Dict] = None):
        ...
        try:
            ...
            state = {
                ...
                'adaptive_parameters': {
                    'max_positions': self.current_max_positions,
                    'pullback': self.current_pullback,
                    'call_retracements': self.previous_call_retracements,
                    'put_retracements': self.previous_put_retracements,
                    'avg_retracement': self.previous_avg_retracement
                }
            }
    ...
    def get_market_data(self, market: str) -> Optional[Dict]:
        """Get real prices from local JSON files"""
        try:
            import json
            ...
            return {
                ...
                'spread': abs(float(call_ask) - float(put_ask)),
    ...
    def display_status(self, market_data: Dict):
        ...
        status += f"| CALL: ${market_data['call_ask']:.3f} (max: ${self.max_call_ask:.3f}) "
        status += f"| PUT: ${market_data['put_ask']:.3f} (max: ${self.max_put_ask:.3f}) "
        status += f"| Balance: {self.call_positions}C/{self.put_positions}P "
        status += f"| MAX: {self.current_max_positions} "
        status += f"| PB: ${self.current_pullback:.3f} "
    ...
    def calculate_retracements_from_trades(self) -> Dict:
        """Calculate number and average width of retracements from trade entry prices"""
        if len(self.positions) == 0:
            return {'call': {'count': 0, 'avg_width': 0.0}, 'put': {'count': 0, 'avg_width': 0.0}}

        # Get CALL and PUT entry prices in chronological order
        call_prices = [p.entry_price for p in self.positions if p.side == 'CALL']
        put_prices = [p.entry_price for p in self.positions if p.side == 'PUT']

        def count_retracements(prices: List[float], min_width: float = 0.04) -> Dict:
            """Count retracements in price series"""
            if len(prices) < 2:
                return {'count': 0, 'avg_width': 0.0}

            retracements = []
            for i in range(1, len(prices)):
                width = abs(prices[i] - prices[i-1])
                if width >= min_width:
                    retracements.append(width)
            ...
            return {
                ...
                'avg_width': sum(retracements) / len(retracements)
            }

        call_stats = count_retracements(call_prices)
        put_stats = count_retracements(put_prices)

        return {'call': call_stats, 'put': put_stats}
    ...
    def check_period_end(self):
        ...
        if final is not None and final != self.final_price:
            ...
            # Calculate retracements and update adaptive parameters for NEXT period
            retracement_stats = self.calculate_retracements_from_trades()
            call_ret = retracement_stats['call']
            put_ret = retracement_stats['put']

            logger.info(f"📊 Retracement Analysis:")
            logger.info(f"   CALL: {call_ret['count']} retracements, avg width: ${call_ret['avg_width']:.3f}")
            logger.info(f"   PUT: {put_ret['count']} retracements, avg width: ${put_ret['avg_width']:.3f}")

            # Update adaptive parameters for next period
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # Update max prices
                if call_ask > self.max_call_ask:
                    self.max_call_ask = call_ask

                if put_ask > self.max_put_ask:
                    self.max_put_ask = put_ask

                # Check for CALL entry (using ADAPTIVE pullback)
                if self.call_positions < self.current_max_positions:
                    pullback_weighted = self.current_pullback * choppiness / 10
                    if self.max_call_ask > 0 and call_ask <= (self.max_call_ask - pullback_weighted):
                        print()  # New line before order log
                        logger.info(f"🔵 CALL entry signal: Price ${call_ask:.3f} is ${pullback_weighted:.3f} below max ${self.max_call_ask:.3f}")
                        logger.info(f"   (Adaptive: pullback=${self.current_pullback:.3f}, max_pos={self.current_max_positions})")
                        if self.open_position(market, 'BUY', call_ask, choppiness=choppiness):
                            logger.info(f"🔵 CALL opened. Resetting max_call_ask.")

                # Check for PUT entry (using ADAPTIVE pullback)
                if self.put_positions < self.current_max_positions:
                    pullback_weighted = self.current_pullback * choppiness / 10
                    if self.max_put_ask > 0 and put_ask <= (self.max_put_ask - pullback_weighted):
                        print()  # New line before order log
                        logger.info(f"🔴 PUT entry signal: Price ${put_ask:.3f} is ${pullback_weighted:.3f} below max ${self.max_put_ask:.3f}")
                        logger.info(f"   (Adaptive: pullback=${self.current_pullback:.3f}, max_pos={self.current_max_positions})")
                        if self.open_position(market, 'SELL', put_ask, choppiness=choppiness):
                            logger.info(f"🔴 PUT opened. Resetting max_put_ask.")
