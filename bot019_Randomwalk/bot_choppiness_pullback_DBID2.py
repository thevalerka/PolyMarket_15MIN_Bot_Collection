#!/usr/bin/env python3
"""
Strategy DBID - Adaptive Choppiness Trading with Bid/Ask Execution
...
pm2 start bot_choppiness_pullback_DBID2.py --cron-restart="00 */6 * * *" --interpreter python3
"""
...
class ChoppinessPullbackBot:
    def __init__(self):
        ...
        # Track retracement stats from previous period for adaptation
        ...
        self.trades_dir = f"{self.data_dir}/tradesDBID2"
        self.state_file = f"{self.data_dir}/stateDBID2.json"
        ...
        logger.info(f"✓ STRATEGY DBID2 - ADAPTIVE with BID/ASK EXECUTION")
        ...
        logger.info(f"✓ Adaptation: Trigger on ASK pullback, Execute at BID")
        logger.info(f"✓ Formula: MAX_POS = min(call_ret, put_ret), PULLBACK = avg_ret - 0.01")
    ...
    def update_adaptive_parameters(self, call_stats: Dict, put_stats: Dict):
        """Update adaptive parameters based on retracement analysis
        call_stats/put_stats contain: {'count': int, 'avg_width': float}
        """
        call_count = call_stats['count']
        put_count = put_stats['count']
        call_avg = call_stats['avg_width']
        put_avg = put_stats['avg_width']

        # Calculate new max positions: minimum of call and put retracements
        # Ensure at least 1 position per side
        new_max_positions = max(1, min(call_count, put_count)) if (call_count > 0 and put_count > 0) else MAX_POSITIONS_PER_SIDE

        # Calculate new pullback: average of call and put avg widths, minus 0.01
        # Ensure pullback is at least 0.05
        combined_avg = (call_avg + put_avg) / 2 if (call_avg > 0 and put_avg > 0) else \
                       (call_avg if call_avg > 0 else put_avg if put_avg > 0 else PULLBACK_AMOUNT)
        new_pullback = max(0.05, combined_avg - 0.01)

        logger.info("=" * 80)
        logger.info("🔧 ADAPTIVE PARAMETER UPDATE")
        logger.info(f"Previous period retracements: CALL={call_count}, PUT={put_count}")
        logger.info(f"Previous period avg widths: CALL=${call_avg:.3f}, PUT=${put_avg:.3f}")
        ...
        self.previous_call_retracements = call_count
        self.previous_put_retracements = put_count
    ...
    def force_save_period(self, final_price: float):
        ...
        # Update adaptive parameters for NEXT period based on CALL/PUT ASK retracements
        call_ask_stats = period_data['retracements']['call_ask']
        put_ask_stats = period_data['retracements']['put_ask']
                ...
                if current_minute >= start_min and current_minute < start_min + 15:
                    ...
                    if response.status_code == 200:
                        ...
                        if data and len(data) > 0:
                            ...
                            # NEW PERIOD DETECTED - save previous period before resetting!
                            if self.period_start and self.period_start != period_start:
                                logger.info(f"\n🔔 Period change: {self.period_start.strftime('%H:%M')} → {period_start.strftime('%H:%M')}")
                                if len(self.positions) > 0:
                                    logger.info(f"💾 Saving {len(self.positions)} positions from previous period")
                                    # The new period's strike = previous period's final
                                    self.force_save_period(strike)
                                else:
                                    logger.info("📭 No positions to save from previous period")
    ...
    def open_position(self, market: str, side: str, price: float, size: float = 5.0, choppiness: float = 0.0) -> bool:
        """Simulate opening a position with balance enforcement and total trades limit"""
        try:
            position_side = 'CALL' if side == 'BUY' else 'PUT'

            # Check max total trades per period (20 max)
            if len(self.positions) >= 20:
                logger.info(f"⚠️  {position_side} rejected: max 20 trades per period reached")
                return False
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # Check for CALL entry (ASK dropped from max, but enter at BID - using ADAPTIVE params)
                if self.call_positions < self.current_max_positions:
                    pullback_weighted = (self.current_pullback + PULLBACK_AMOUNT*choppiness/10)/2

                    if self.call_positions == self.put_positions :
                        pullback_weighted = (self.current_pullback + PULLBACK_AMOUNT*choppiness/10)/2

                    if self.call_positions < self.put_positions :
                        pullback_weighted = (self.current_pullback + PULLBACK_AMOUNT*choppiness/10)/4

                    if self.call_positions > self.put_positions :   # questo non dovrebbe esistere
                        pullback_weighted = (self.current_pullback + PULLBACK_AMOUNT*choppiness/10)/1.5
                ...
                # Check for PUT entry (ASK dropped from max, but enter at BID - using ADAPTIVE params)
                if self.put_positions < self.current_max_positions:
                    ...
                    if self.put_positions > self.call_positions :   # questo non dovrebbe esistere
                        pullback_weighted = (self.current_pullback + PULLBACK_AMOUNT*choppiness/10)/1.5
