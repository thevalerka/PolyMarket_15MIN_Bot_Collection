...
- Trade only when choppiness > 30
- Max 3 CALL positions and max 3 PUT positions
- Entry: when ask price drops 0.05$ below recent max
- Reset max after each position opened
- Real-time PNL calculation using Binance strike and final prices
- SIMULATION ONLY - No real orders placed
pm2 start bot_choppiness_pullback_C2.py --cron-restart="00 */6 * * *" --interpreter python3
"""
...
class ChoppinessPullbackBot:
    def __init__(self):
        ...
        self.trades_dir = f"{self.data_dir}/tradesC2"
        self.state_file = f"{self.data_dir}/stateC2.json"
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # Check for CALL entry (price dropped 0.05 from max)
                if self.call_positions < MAX_POSITIONS_PER_SIDE:
                    ...
                    if self.call_positions < self.put_positions :
                        pullback_weighted = PULLBACK_AMOUNT*choppiness/20

                    if self.call_positions > self.put_positions :   # questo non dovrebbe esistere
                        pullback_weighted = PULLBACK_AMOUNT*choppiness/5

                    if self.max_call_ask > 0 and call_ask <= (self.max_call_ask - pullback_weighted):
                        print()  # New line before order log
                        logger.info(f"🔵 CALL entry signal: Price ${call_ask:.3f} is ${pullback_weighted} below max ${self.max_call_ask:.3f}")
                        if self.open_position(market, 'BUY', call_ask, choppiness=choppiness):
                            logger.info(f"🔵 CALL opened. Resetting max_call_ask. Next entry on new high -$0.05")

                # Check for PUT entry (price dropped 0.05 from max)
                if self.put_positions < MAX_POSITIONS_PER_SIDE:
                    ...
                    if self.max_put_ask > 0 and put_ask <= (self.max_put_ask - pullback_weighted):
                        print()  # New line before order log
                        logger.info(f"🔴 PUT entry signal: Price ${put_ask:.3f} is ${pullback_weighted} below max ${self.max_put_ask:.3f}")
                        if self.open_position(market, 'SELL', put_ask, choppiness=choppiness):
                            logger.info(f"🔴 PUT opened. Resetting max_put_ask. Next entry on new high -$0.05")
