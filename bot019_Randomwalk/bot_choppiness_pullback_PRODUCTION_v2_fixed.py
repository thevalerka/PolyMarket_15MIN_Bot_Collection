...
def calculate_choppiness_index(prices: deque, period: int = 14) -> float:
    """
    Calculate Choppiness Index - EXACTLY matches simulator formula
    ...
    if len(prices) < period:
        return 0.0  # Return 0 if not enough data (matches simulator)
    
    # Get last 'period' prices (NOT period+1!)
    prices_list = list(prices)[-period:]
    
    # For tick data, highs/lows/closes are all the same price
    highs = prices_list
    lows = prices_list
    closes = prices_list
    ...
    # CRITICAL: Loop starts at i=1 and uses closes[i-1]
    # This means we compare current bar (highs[i]) to previous close (closes[i-1])
    atr_sum = 0.0
    ...
    if high_max - low_min == 0:
        return 0.0  # Return 0 (matches simulator)
