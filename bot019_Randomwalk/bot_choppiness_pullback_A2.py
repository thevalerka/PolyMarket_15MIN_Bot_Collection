...
pm2 start bot_choppiness_pullback_A2.py --cron-restart="00 * * * *" --interpreter python3
"""
...
class ChoppinessPullbackBot:
    def __init__(self):
        ...
        self.trades_dir = f"{self.data_dir}/tradesA2"
        self.state_file = f"{self.data_dir}/stateA2.json"
        ...
        while True:
            try:
                ...
                if self.call_positions < MAX_POSITIONS_PER_SIDE:
                    if self.max_call_ask > 0 and call_ask <= (self.max_call_ask - PULLBACK_AMOUNT):
                        print()  # New line before order log
                        logger.info(f"🔵 CALL entry signal: Price ${call_ask:.3f} is ${PULLBACK_AMOUNT} below max ${self.max_call_ask:.3f}")
                ...
                if self.put_positions < MAX_POSITIONS_PER_SIDE:
                    if self.max_put_ask > 0 and put_ask <= (self.max_put_ask - PULLBACK_AMOUNT):
                        print()  # New line before order log
                        logger.info(f"🔴 PUT entry signal: Price ${put_ask:.3f} is ${PULLBACK_AMOUNT} below max ${self.max_put_ask:.3f}")
