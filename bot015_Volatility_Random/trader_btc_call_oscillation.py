#!/usr/bin/env python3
"""
15-Minute Binary Options BTC_CALL Trader - Oscillation-Based Strategy

Strategy:
- Trade ONLY BTC_CALL
- Track second-by-second price data for entire 15-minute period
- Calculate price oscillation (range = max - min) over 15 seconds
- Compare 15s oscillation to period average (calculated from period start)
- ONLY TRADE when 15s oscillation is HIGH (above average AND above threshold)

Entry Rules:
- Buy 5 shares when:
  1. We hold 0 assets
  2. 15s oscillation > period average oscillation
  3. 15s oscillation >= 0.042 (minimum threshold)
  4. Ask price <= $0.96 (never buy above)
  5. Sufficient USDC balance

Exit Rules:
- Take Profit: Sell when bid_price >= (5% OR $0.03 above buy, whichever is greater)
- Stop Loss: Sell when bid_price <= (10% OR $0.07 below buy, whichever is greater)
...
Safety:
- Multi-phase verification (1s, 5s, 10s, 20s)
- Check both USDC (ERC-20) and token (ERC-1155) balances
- Cancel all orders if position not confirmed
...
# pm2 start trader_btc_call_oscillation.py --cron-restart="00 * * * *" --interpreter python3



class BTCCallVolatilityTrader:
    """BTC_CALL trader with oscillation-based entry filtering"""
    ...
    TAKE_PROFIT_MIN_DOLLARS = 0.05  # Minimum $0.03 profit
    STOP_LOSS_PERCENT = 0.10  # 10% loss
    STOP_LOSS_MIN_DOLLARS = 0.1  # Minimum $0.07 loss

    # Price limits
    MAX_BUY_PRICE = 0.96  # Never buy above this price
    EMERGENCY_SELL_PRICE = 0.99  # Always sell at this price

    # Oscillation tracking
    MAX_PRICE_HISTORY = 900  # Store up to 15 minutes of second-by-second data
    MIN_OSCILLATION_THRESHOLD = 0.062  # Minimum absolute oscillation required to trade

    # Oscillation window (in seconds)
    OSCILLATION_WINDOW = 15  # Use 15 seconds as the sole metric
    ...
    def __init__(self, credentials: Dict[str, str]):
        ...
        # Price history for oscillation calculation (deque for efficient append/pop)
        self.price_history: Deque[float] = deque(maxlen=self.MAX_PRICE_HISTORY)

        # Oscillation tracking (15 seconds only)
        self.current_oscillation = 0.0  # Current 15s oscillation
        self.period_oscillations = []  # Track oscillation values throughout current period
        self.period_average_oscillation = 0.0  # Average oscillation for current period
        self.historical_oscillation = []  # Historical period average oscillations

        # Load historical oscillation data
        self._load_oscillation_history()

        print("=" * 80)
        print("🎯 BTC_CALL OSCILLATION-BASED TRADER")
        print("=" * 80)
        print(f"📊 Asset: BTC_CALL only")
        print(f"📦 Position Size: {self.POSITION_SIZE} shares")
        print(f"📈 Take Profit: 5% OR $0.03 above buy price (whichever is greater)")
        print(f"📉 Stop Loss: 10% OR $0.07 below buy price (whichever is greater)")
        print(f"🚫 Max Buy Price: ${self.MAX_BUY_PRICE} (never buy above)")
        print(f"🚨 Emergency Sell: ${self.EMERGENCY_SELL_PRICE} (always sell at or above)")
        print(f"🌊 Oscillation Filter: 15s oscillation must be > period average AND >= {self.MIN_OSCILLATION_THRESHOLD}")
        print(f"📊 Oscillation = price range (max - min) over 15 seconds")
    ...
    def _load_oscillation_history(self):
        """Load historical oscillation data (not currently used for trading decisions)"""
        try:
            if os.path.exists(self.VOLATILITY_HISTORY_FILE):
                with open(self.VOLATILITY_HISTORY_FILE, 'r') as f:
                    data = json.load(f)
                    self.historical_oscillation = data.get('history', [])

                if len(self.historical_oscillation) > 0:
                    print(f"✅ Loaded {len(self.historical_oscillation)} historical period averages")
                else:
                    print("⚠️  No historical oscillation data found")
            else:
                print("⚠️  Oscillation history file not found - will create on first period")

        except Exception as e:
            print(f"⚠️  Error loading oscillation history: {e}")

    def _save_oscillation_history(self):
        """Save oscillation history to file"""
        try:
            # Keep only last 100 periods (to avoid file growing indefinitely)
            if len(self.historical_oscillation) > 100:
                self.historical_oscillation = self.historical_oscillation[-100:]

            data = {
                'history': self.historical_oscillation,
            ...
            print(f"⚠️  Error saving oscillation history: {e}")
    ...
    def calculate_oscillation(self) -> float:
        """
        Calculate oscillation (price range) over 15 second window

        Oscillation = (max_price - min_price) in the 15 second window
        This measures how much the price is actually moving/swinging

        Returns:
            Oscillation value for 15 seconds
        """
        if len(self.price_history) < self.OSCILLATION_WINDOW:
            return 0.0

        # Get prices for 15 second window (last 15 seconds)
        window_prices = list(self.price_history)[-self.OSCILLATION_WINDOW:]
    ...
    def update_price_history(self, mid_price: float):
        """Add new price to history and update oscillation metric"""
        self.price_history.append(mid_price)

        # Calculate 15s oscillation
        self.current_oscillation = self.calculate_oscillation()

        # Track oscillation throughout the period (if we have valid data)
        if self.current_oscillation > 0 and len(self.price_history) >= self.OSCILLATION_WINDOW:
            self.period_oscillations.append(self.current_oscillation)
            # Calculate running average for the period
            self.period_average_oscillation = statistics.mean(self.period_oscillations)
    ...
    def execute_buy(self, token_id: str, ask_price: float) -> bool:
        ...
        try:
            ...
            if position_confirmed and final_balance >= 0.5:
                ...
                take_profit_percent_price = ask_price * (1 + self.TAKE_PROFIT_PERCENT)
                take_profit_dollar_price = ask_price + self.TAKE_PROFIT_MIN_DOLLARS
                take_profit_price = max(take_profit_percent_price, take_profit_dollar_price)

                stop_loss_percent_price = ask_price * (1 - self.STOP_LOSS_PERCENT)
                stop_loss_dollar_price = ask_price - self.STOP_LOSS_MIN_DOLLARS
                stop_loss_price = min(stop_loss_percent_price, stop_loss_dollar_price)

                print(f"\n📊 POSITION OPENED")
                print(f"   Size: {final_balance:.2f} shares")
                print(f"   Avg Buy Price: ${ask_price:.4f}")
                print(f"   Take Profit Target: ${take_profit_price:.4f}")
                print(f"   Stop Loss Target: ${stop_loss_price:.4f}")
    ...
    def check_exit_conditions(self, bid_price: float) -> Optional[str]:
        ...
        Returns:
            Reason for exit or None if no exit
        """
        ...
        # EMERGENCY SELL: Always sell at or above $0.99
        if bid_price >= self.EMERGENCY_SELL_PRICE:
            return f'EMERGENCY SELL (price >= ${self.EMERGENCY_SELL_PRICE})'

        # Take profit: 5% OR $0.03 above buy (whichever gives higher sell price)
        take_profit_percent_price = avg_buy * (1 + self.TAKE_PROFIT_PERCENT)
        take_profit_dollar_price = avg_buy + self.TAKE_PROFIT_MIN_DOLLARS
        take_profit_price = max(take_profit_percent_price, take_profit_dollar_price)

        if bid_price >= take_profit_price:
            profit_pct = ((bid_price / avg_buy) - 1) * 100
            profit_dollars = bid_price - avg_buy
            return f'TAKE-PROFIT ({profit_pct:.2f}% / ${profit_dollars:.3f} profit)'

        # Stop loss: 10% OR $0.07 below buy (whichever gives lower sell price)
        stop_loss_percent_price = avg_buy * (1 - self.STOP_LOSS_PERCENT)
        stop_loss_dollar_price = avg_buy - self.STOP_LOSS_MIN_DOLLARS
        stop_loss_price = min(stop_loss_percent_price, stop_loss_dollar_price)

        if bid_price <= stop_loss_price:
            loss_pct = ((bid_price / avg_buy) - 1) * 100
            loss_dollars = bid_price - avg_buy
            return f'STOP-LOSS ({loss_pct:.2f}% / ${loss_dollars:.3f} loss)'
    ...
    def check_entry_conditions(self, data: Dict) -> bool:
        ...
        # Need at least 15 seconds of data to calculate meaningful oscillation
        if len(self.price_history) < self.OSCILLATION_WINDOW:
            return False

        # Need at least some oscillation samples to calculate period average
        if len(self.period_oscillations) < 10:
            return False

        # Check oscillation - must be both above period average AND above minimum threshold
        if self.current_oscillation <= self.period_average_oscillation:
            return False

        if self.current_oscillation < self.MIN_OSCILLATION_THRESHOLD:
            return False

        # Check ask price and balance
        ask_price = float(data['best_ask'].get('price', 0))
        if ask_price == 0:
            return False

        # Never buy above max price
    ...
    def run_trading_cycle(self):
        ...
        try:
            ...
            # Display 15s oscillation
            print(f"🌊 15s Oscillation: {self.current_oscillation:.6f} | Period Avg: {self.period_average_oscillation:.6f} | Min: {self.MIN_OSCILLATION_THRESHOLD}", end="")

            # Highlight oscillation status
            above_avg = self.current_oscillation > self.period_average_oscillation
            above_min = self.current_oscillation >= self.MIN_OSCILLATION_THRESHOLD

            if above_avg and above_min:
                ratio = self.current_oscillation / self.period_average_oscillation if self.period_average_oscillation > 0 else 0
            ...
            print(f"📊 Price History: {len(self.price_history)} data points | Period Osc Samples: {len(self.period_oscillations)}")
            ...
            if self.current_position['size'] >= 0.5:
                ...
                print(f"   Emergency Sell: ${self.EMERGENCY_SELL_PRICE:.4f} (need {((self.EMERGENCY_SELL_PRICE/bid_price)-1)*100:+.2f}% move)")
                ...
                if self.check_entry_conditions(data):
                    ...
                    print(f"   15s Oscillation: {self.current_oscillation:.6f} > Period Avg: {self.period_average_oscillation:.6f}")
                    print(f"   15s Oscillation: {self.current_oscillation:.6f} >= Min: {self.MIN_OSCILLATION_THRESHOLD}")
                    print(f"   Ask Price: ${ask_price:.4f} <= Max: ${self.MAX_BUY_PRICE}")
                    self.execute_buy(data['asset_id'], ask_price)
                else:
                    if len(self.price_history) < self.OSCILLATION_WINDOW:
                        print(f"\n⏸️  Waiting for more data (have {len(self.price_history)} prices, need {self.OSCILLATION_WINDOW})")
                    elif len(self.period_oscillations) < 10:
                        print(f"\n⏸️  Waiting for more oscillation samples (have {len(self.period_oscillations)}, need 10)")
                    elif ask_price > self.MAX_BUY_PRICE:
                        print(f"\n⏸️  Price too high (ask: ${ask_price:.4f} > max: ${self.MAX_BUY_PRICE})")
                    elif self.current_oscillation < self.MIN_OSCILLATION_THRESHOLD:
                        print(f"\n⏸️  Waiting for higher oscillation (15s: {self.current_oscillation:.6f} < min: {self.MIN_OSCILLATION_THRESHOLD})")
                    elif self.current_oscillation <= self.period_average_oscillation:
                        print(f"\n⏸️  Waiting for higher oscillation (15s: {self.current_oscillation:.6f} <= period avg: {self.period_average_oscillation:.6f})")

            # Check for new period (reset oscillation tracking)
            if time_left > 14.5:
                print("\n🔄 NEW PERIOD DETECTED")

                # Save average oscillation of completed period if we have data
                if len(self.period_oscillations) > 100:  # Only save if we have substantial data
                    period_avg = self.period_average_oscillation
                    self.historical_oscillation.append(period_avg)
                    print(f"   Saved period average oscillation: {period_avg:.6f}")
                    print(f"   Total historical periods: {len(self.historical_oscillation)}")

                    # Save to file
                    self._save_oscillation_history()

                # Clear tracking for new period
                self.price_history.clear()
                self.period_oscillations.clear()
                self.current_oscillation = 0.0
                self.period_average_oscillation = 0.0
                print(f"   Price history and period oscillations cleared for new period")
    ...
    def run(self):
        ...
        print("\n🚀 Starting BTC_CALL oscillation-based trader...")
        print(f"📊 Asset: BTC_CALL only")
        print(f"🌊 Strategy: 15s oscillation must be > period average AND >= {self.MIN_OSCILLATION_THRESHOLD}")
        print(f"📊 Oscillation = price range (max - min) over 15 seconds")
        print(f"📊 Period average calculated from beginning of each 15-min period")
        print(f"📦 Position: {self.POSITION_SIZE} shares")
        print(f"📈 Take Profit: 5% OR $0.03 above buy (whichever is greater)")
        print(f"📉 Stop Loss: 10% OR $0.07 below buy (whichever is greater)")
        print(f"⚡ Monitoring: Every second")
        print("\n" + "=" * 80 + "\n")
