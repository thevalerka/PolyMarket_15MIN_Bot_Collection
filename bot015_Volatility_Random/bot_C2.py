#!/usr/bin/env python3
"""
Strategy C2 - ATR-Based Binary Options BTC_CALL Trader

Strategy C2 Characteristics:
- ATR-based dynamic TP/SL calculation
- TP:SL ratio of 1:2 (more aggressive TP, wider SL)
...
- Stop Loss: Dynamic based on ATR (1:2 ratio with TP)
...
pm2 start bot_C2.py --cron-restart="00 * * * *" --interpreter python3
"""
...
class StrategyC2Trader:
    """BTC_CALL trader with Strategy C2 (ATR-based, 1:2 ratio)"""
    ...
    def calculate_dynamic_tpsl(self, atr: float, entry_price: float) -> Tuple[float, float]:
        """
        Calculate dynamic TP and SL for Strategy C2
        Maintains 1:2 ratio (TP:SL)
        """
        ...
        tp_distance = max(0.02, min(0.20, tp_distance))
    ...
    def execute_buy(self, token_id: str, ask_price: float, take_profit: float,
        ...
        Uses ORIGINAL WORKING LOGIC from trader_btc_call_oscillation.py
        """
        try:
            print(f"\n{'='*70}")
            print(f"🛒 EXECUTING BUY ORDER - STRATEGY C2")
            print(f"{'='*70}")
    ...
    def run_trading_cycle(self):
        """Main trading loop"""
        print(f"\n{'='*70}")
        print(f"🚀 STARTING STRATEGY C2 TRADING CYCLE")
        print(f"{'='*70}\n")
...
def main():
    ...
    trader = StrategyC2Trader(credentials)
