...
class Strategy(Enum):
    C = "C"      # ATR-based dynamic TP/SL, 2:3 ratio (TP:SL)
    C2 = "C2"    # ATR-based dynamic TP/SL, 1:2 ratio (more aggressive TP)
    C3 = "C3"    # C with price-based distance multipliers
    D = "D"      # C with dynamic ATR threshold based on PNL performance
...
class BinaryOptionsTrader:
    def __init__(self):
        ...
        # Strategy D: Dynamic ATR threshold
        self.strategy_d_atr_threshold = 0.0
        self.strategy_d_last_pnl = 0.0
    ...
    def get_dynamic_tpsl(self, atr: float, entry_price: float) -> Tuple[float, float]:
        """
        Calculate dynamic TP and SL based on ATR for Strategy C
    ...
    def get_dynamic_tpsl_c2(self, atr: float, entry_price: float) -> Tuple[float, float]:
        """
        Calculate dynamic TP and SL based on ATR for Strategy C2
        Maintains 1:2 ratio (TP:SL) - More aggressive take profit
        ...
        # Scale the TP/SL based on ATR and expected magnitude
        # Base TP/SL with 1:2 ratio
        base_tp = 0.03  # Minimum TP
        base_sl = 0.06  # Minimum SL (maintains 1:2 ratio)
        
        # Scale based on ATR-derived expected magnitude and probability
        scaling_factor = expected_magnitude * prob_movement
        
        tp_distance = base_tp + (scaling_factor * 1.0)
        sl_distance = base_sl + (scaling_factor * 2.0)
        
        # Ensure minimum values and maintain ratio
        tp_distance = max(0.02, min(0.20, tp_distance))  # Clamp TP
        sl_distance = tp_distance * 2.0  # Maintain 1:2 ratio
    ...
    def get_price_multiplier(self, price: float) -> float:
        """
        Get distance multiplier based on price level for Strategy C3
    ...
    def get_dynamic_tpsl_c3(self, atr: float, entry_price: float) -> Tuple[float, float]:
        """
        Calculate dynamic TP and SL based on ATR for Strategy C3
        Maintains 2:3 ratio (TP:SL) with price-based distance multipliers
        ...
        # Apply price-based multiplier
        price_multiplier = self.get_price_multiplier(entry_price)
        tp_distance *= price_multiplier
        sl_distance *= price_multiplier
    ...
    def check_trade_conditions(self, data: Dict[str, Dict], current_time: datetime, 
        ...
        # Calculate ATR for all C-based strategies
        ...
        has_strategy_c2 = any(p.strategy == 'C2' and p.asset_type == 'BTC_CALL' for p in self.positions)
        has_strategy_c3 = any(p.strategy == 'C3' and p.asset_type == 'BTC_CALL' for p in self.positions)
        has_strategy_d = any(p.strategy == 'D' and p.asset_type == 'BTC_CALL' for p in self.positions)
        
        if atr_value is not None:
            # Strategy C: Dynamic TP/SL based on ATR (2:3 ratio)
            ...
            # Strategy C2: Dynamic TP/SL based on ATR (1:2 ratio - more aggressive TP)
            if not has_strategy_c2:
                tp, sl = self.get_dynamic_tpsl_c2(atr_value, btc_call_ask)
                tp = min(0.99, tp)
                signals.append(('BTC_CALL', btc_call_ask, period_end, 'C2', tp, sl, atr_value))
            
            # Strategy C3: C with price-based multipliers
            if not has_strategy_c3:
                tp, sl = self.get_dynamic_tpsl_c3(atr_value, btc_call_ask)
                tp = min(0.99, tp)
                signals.append(('BTC_CALL', btc_call_ask, period_end, 'C3', tp, sl, atr_value))
            
            # Strategy D: C with dynamic ATR threshold
            if not has_strategy_d:
                # Only enter if ATR is above the current threshold
                if atr_value >= self.strategy_d_atr_threshold:
                    tp, sl = self.get_dynamic_tpsl(atr_value, btc_call_ask)
                    tp = min(0.99, tp)
                    signals.append(('BTC_CALL', btc_call_ask, period_end, 'D', tp, sl, atr_value))
    ...
    def open_position(self, asset_type: str, entry_price: float, entry_time: datetime,
        ...
        # For Strategy C variants, display ATR metrics
        if strategy in ['C', 'C2', 'C3', 'D'] and atr_value is not None:
            prob_movement, expected_magnitude = self.atr_calc.estimate_movement_probability(
            ...
            if strategy == 'C':
                print(f"   TP/SL Ratio: 2:3 (standard)")
            elif strategy == 'C2':
                print(f"   TP/SL Ratio: 1:2 (aggressive TP)")
            elif strategy == 'C3':
                price_mult = self.get_price_multiplier(entry_price)
                print(f"   TP/SL Ratio: 2:3 (standard)")
                print(f"   Price Multiplier: {price_mult}x (price ${entry_price:.2f})")
            elif strategy == 'D':
                print(f"   TP/SL Ratio: 2:3 (standard)")
                print(f"   ATR Threshold: ${self.strategy_d_atr_threshold:.4f}")
                print(f"   Current ATR: ${atr_value:.4f} (✓ above threshold)")
        ...
        if atr_value is None and strategy in ['C', 'C2', 'C3', 'D']:
    ...
    def check_tpsl_exit(self, data: Dict[str, Dict], current_time: datetime):
        ...
        for pos in self.positions:
            ...
            # All C-based strategies: Dynamically adjust TP/SL based on current ATR
            if pos.strategy in ['C', 'C2', 'C3', 'D']:
                current_atr = self.calculate_atr(pos.asset_type, period_seconds=15, num_periods=6)
                
                if current_atr is not None:
                    # Get new TP/SL based on strategy type
                    if pos.strategy == 'C' or pos.strategy == 'D':
                        new_tp, new_sl = self.get_dynamic_tpsl(current_atr, pos.entry_price)
                    elif pos.strategy == 'C2':
                        new_tp, new_sl = self.get_dynamic_tpsl_c2(current_atr, pos.entry_price)
                    elif pos.strategy == 'C3':
                        new_tp, new_sl = self.get_dynamic_tpsl_c3(current_atr, pos.entry_price)
    ...
    def close_position(self, pos: Position, data: Dict[str, Dict]):
        ...
        # Strategy D: Adjust ATR threshold based on PNL performance
        if pos.strategy == 'D':
            current_total_pnl = self.total_pnl['D']
            
            if current_total_pnl < self.strategy_d_last_pnl:
                # PNL decreased - increase threshold (less trading)
                old_threshold = self.strategy_d_atr_threshold
                self.strategy_d_atr_threshold += 0.001
                print(f"\n📉 Strategy D: PNL decreased (${self.strategy_d_last_pnl:.4f} → ${current_total_pnl:.4f})")
                print(f"   ATR Threshold increased: ${old_threshold:.4f} → ${self.strategy_d_atr_threshold:.4f}")
            elif current_total_pnl > self.strategy_d_last_pnl:
                # PNL increased - decrease threshold (more trading)
                old_threshold = self.strategy_d_atr_threshold
                self.strategy_d_atr_threshold = max(0.0, self.strategy_d_atr_threshold - 0.001)
                print(f"\n📈 Strategy D: PNL increased (${self.strategy_d_last_pnl:.4f} → ${current_total_pnl:.4f})")
                print(f"   ATR Threshold decreased: ${old_threshold:.4f} → ${self.strategy_d_atr_threshold:.4f}")
            
            self.strategy_d_last_pnl = current_total_pnl
    ...
    def print_status(self):
        ...
        if self.positions:
            ...
            for pos in self.positions:
                ...
                # For C-based strategies, show current ATR metrics
                if pos.strategy in ['C', 'C2', 'C3', 'D']:
                    current_atr = self.calculate_atr(pos.asset_type, period_seconds=15, num_periods=6)
                    if current_atr is not None:
                        prob_movement, expected_magnitude = self.atr_calc.estimate_movement_probability(
                        ...
                        extra_info = ""
                        if pos.strategy == 'C3':
                            price_mult = self.get_price_multiplier(pos.entry_price)
                            extra_info = f", Mult: {price_mult}x"
                        elif pos.strategy == 'D':
                            extra_info = f", Threshold: ${self.strategy_d_atr_threshold:.4f}"
                        
                        print(f"       🎯 ATR: ${current_atr:.4f} ({atr_pct:.2f}%), "
                              f"P(move): {prob_movement:.1%}, "
                              f"Exp.Mag: ${expected_magnitude:.4f}{extra_info}")
        print()
    ...
    def run(self):
        """Main trading loop"""
        print("="*80)
        print("Binary Options Trading Simulator - C Strategies Focus")
        print("="*80)
        print(f"Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("\nAll strategies run continuously on BTC_CALL:")
        print()
        print("  C:   ATR-based dynamic TP/SL, 2:3 ratio (TP:SL)")
        print("       Standard adaptive strategy")
        print()
        print("  C2:  ATR-based dynamic TP/SL, 1:2 ratio (TP:SL)")
        print("       More aggressive - smaller TP, larger SL")
        print()
        print("  C3:  C strategy with price-based distance multipliers:")
        print("       $0.00-0.10: 0.6x | $0.10-0.30: 0.8x | $0.30-0.40: 1.0x")
        print("       $0.40-0.60: 1.2x | $0.60-0.70: 1.0x | $0.70-0.90: 0.8x")
        print("       $0.90-1.00: 0.6x")
        print()
        print("  D:   C strategy with dynamic ATR threshold")
        print("       Starts at threshold = $0.00")
        print("       If PNL decreases → threshold +$0.001 (trade less)")
        print("       If PNL increases → threshold -$0.001 (trade more)")
        print()
        ...
        print()
        print("All strategies based on ATR (6 periods × 15s, exp. weighted)")
        print("Price checks every 1 second for TP/SL exits and adjustments")
