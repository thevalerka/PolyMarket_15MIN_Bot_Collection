...
class BinaryOptionsTrader:
    ...
    def read_oscillation_data(self) -> Optional[Dict]:
        ...
        try:
            with open(self.oscillation_file, 'r') as f:
                data = json.load(f)
                print(f"\n[DEBUG] Successfully read oscillation data from {self.oscillation_file}")
                print(f"[DEBUG] Oscillation data keys: {list(data.keys())}")
                print(f"[DEBUG] Volatility: {data.get('volatility', 'NOT FOUND')}")
                print(f"[DEBUG] Data type: {type(data)}")
                return data
        except FileNotFoundError:
            print(f"\n[ERROR] Oscillation file not found: {self.oscillation_file}")
            return None
        except json.JSONDecodeError as e:
            print(f"\n[ERROR] Error decoding {self.oscillation_file}: {e}")
            return None
        except Exception as e:
            print(f"\n[ERROR] Unexpected error reading oscillation data: {e}")
            return None
    ...
    def open_position(self, asset_type: str, entry_price: float, entry_time: datetime, 
                     period_end: datetime, strategy: str, pair_id: str, market_conditions: Optional[Dict] = None):
        """Open a new position"""
    ...
    def save_results(self):
        ...
        print(f"[DEBUG] Number of trades: {len(self.trades)}")
        print(f"[DEBUG] Number of open positions: {len(self.positions)}")
        ...
        for i, trade in enumerate(self.trades):
            ...
            print(f"[DEBUG] Trade {i}: market_conditions is None: {trade.market_conditions is None}")
            if trade.market_conditions:
                print(f"[DEBUG] Trade {i}: market_conditions keys: {list(trade.market_conditions.keys())}")
        ...
        for i, pos in enumerate(self.positions):
            ...
            print(f"[DEBUG] Position {i}: market_conditions is None: {pos.market_conditions is None}")
            if pos.market_conditions:
                print(f"[DEBUG] Position {i}: market_conditions keys: {list(pos.market_conditions.keys())}")
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Read oscillation data once per iteration if there are signals
                if trade_signals:
                    print(f"\n[DEBUG] Found {len(trade_signals)} trade signal(s), reading oscillation data...")
                    oscillation_data = self.read_oscillation_data()
                    
                    if oscillation_data:
                        print(f"[DEBUG] Oscillation data successfully loaded")
                        print(f"[Market Conditions Captured]")
                        print(f"  Volatility: {oscillation_data.get('volatility', 'N/A')}")
                        print(f"  Time to Expiry: {oscillation_data.get('time_to_expiry_minutes', 'N/A')} min")
                        print(f"  Efficiency Ratio: {oscillation_data.get('oscillation_60min', {}).get('efficiency_ratio', 'N/A')}")
                    else:
                        print(f"[DEBUG] WARNING: oscillation_data is None!")
                    
                    for signal in trade_signals:
                        asset1, price1, asset2, price2, period_end, strategy, pair_type = signal
                        
                        print(f"\n[DEBUG] Processing signal for {pair_type} with strategy {strategy}")
                        print(f"[DEBUG] About to call open_position with oscillation_data: {oscillation_data is not None}")
                        ...
                        # Open both positions with market conditions
                        self.open_position(asset1, price1, current_time, period_end, strategy, pair_id, oscillation_data)
                        self.open_position(asset2, price2, current_time, period_end, strategy, pair_id, oscillation_data)
