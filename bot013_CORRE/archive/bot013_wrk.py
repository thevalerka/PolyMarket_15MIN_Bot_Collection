...
class PairTrader:
    ...
    def execute_pair_trade(self, pair_info: Dict, num_pairs: int) -> bool:
        ...
        print(f"📤 Placing sell order for PUT @ ${sell_price}...")
        put_sell_order_id = self.trader.place_sell_order(
            token_id=pair_info['put_token_id'],
            price=sell_price,
            quantity=position_size
        )

        if put_sell_order_id:
            print(f"✅ PUT sell order placed @ ${sell_price}")
        else:
            print(f"⚠️  PUT sell order failed - position still held")

        time.sleep(0.5)

        # LEG 2: Buy CALL (MANDATORY - even if conditions changed)
        ...
        if not call_order_id:
            print("❌ WARNING: LEG 2 (CALL) failed but LEG 1 is open!")
            print("⚠️  Manual intervention may be needed")
            return False

        print(f"✅ LEG 2 executed: {pair_info['coin2']} CALL")

        # Immediately place sell order for CALL at $0.99
        #time.sleep(1)  # Brief pause to let buy order settle
        print(f"📤 Placing sell order for CALL @ ${sell_price}...")
        call_sell_order_id = self.trader.place_sell_order(
            token_id=pair_info['call_token_id'],
            price=sell_price,
            quantity=position_size
        )

        if call_sell_order_id:
            print(f"✅ CALL sell order placed @ ${sell_price}")
        else:
            print(f"⚠️  CALL sell order failed - position still held")
