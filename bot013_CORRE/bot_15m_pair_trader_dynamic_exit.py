#!/usr/bin/env python3
"""
15-Minute Binary Options Pair Trading Bot - Dynamic Exit Strategy
...
- Sells BOTH assets of a pair when:
  ...
  * asset1_bid + asset2_bid > $0.94 (take profit)
"""
...
class PairTrader:
    """15-minute binary options pair trading bot with dynamic exit strategy"""
    ...
    def __init__(self, credentials: Dict[str, str]):
        ...
        print("=" * 70)
        print("🎯 15-MINUTE PAIR TRADING BOT - DYNAMIC EXIT STRATEGY")
        print("=" * 70)
    ...
    def verify_sell_order_exists(self, token_id: str) -> bool:
        """Check if a sell order exists for a token"""
        try:
            from py_clob_client.clob_types import OrderArgs

            # Get open orders for this token
            orders = self.trader.client.get_orders(
                OrderArgs(asset_id=token_id)
            )

            # Check if there's an active sell order
            for order in orders:
                if order.get('side') == 'SELL' and order.get('status') in ['LIVE', 'ACTIVE']:
                    return True
    ...
    def place_immediate_market_sell(self, token_id: str, size: float, coin: str, option_type: str) -> bool:
        ...
        try:
            ...
            # FIXED: Using place_sell_order instead of place_order
    ...
    def execute_pair_trade(self, pair_info: Dict, num_pairs: int) -> bool:
        ...
        """
        position_size = self.BASE_POSITION_SIZE  # Fixed 5 tokens per leg
    ...
    def scan_all_pairs(self) -> List[Tuple[str, Dict]]:
        ...
        print(f"\n{'='*90}")
        print(f"{'PAIR NAME':<25} | {'ASSET1':<8} | {'ASSET2':<8} | {'COMBINED':<9} | {'TIME TO EXP':<12} | {'STATUS'}")
        print(f"{'='*90}")
        ...
        for i, coin1 in enumerate(self.COINS):
            for coin2 in self.COINS[i+1:]:
                ...
                if pair_info1:
                    ...
                    print(f"{pair_key1:<25} | ${put_ask:<7.4f} | ${call_ask:<7.4f} | ${total:<8.4f} | {time_left:>5.1f} min    | {status}")
                else:
                    ...
                    if put_data and call_data:
                        ...
                        if put_ask == 0 or call_ask == 0:
                            ...
                            if not (self.MIN_ASK_PRICE <= put_ask <= self.MAX_ASK_PRICE):
                                reasons.append(f"PUT ${put_ask:.4f} OUT OF RANGE")
                            if not (self.MIN_ASK_PRICE <= call_ask <= self.MAX_ASK_PRICE):
                                reasons.append(f"CALL ${call_ask:.4f} OUT OF RANGE")
                        ...
                        if put_ask > 0 and call_ask > 0:
                            ...
                            print(f"{pair_key1:<25} | {'N/A':<8} | {'N/A':<8} | {'N/A':<9} | {time_left:>5.1f} min    | {status}")
                ...
                if pair_info2:
                    ...
                    print(f"{pair_key2:<25} | ${put_ask:<7.4f} | ${call_ask:<7.4f} | ${total:<8.4f} | {time_left:>5.1f} min    | {status}")
                else:
                    # Show why it doesn't meet conditions
                    ...
                    if put_data and call_data:
                        ...
                        if put_ask > 0 and call_ask > 0:
                            ...
                            print(f"{pair_key2:<25} | {'N/A':<8} | {'N/A':<8} | {'N/A':<9} | {time_left:>5.1f} min    | {status}")
    ...
    def monitor_and_exit_positions(self):
        """
        ENHANCED: Monitor all open positions and execute exit strategy
    ...
    def run_trading_cycle(self) -> bool:
        ...
        try:
            ...
            # Only look for new entries if we have time AND available balance
                ...
                if usdc_balance >= (self.MIN_ASK_PRICE * 2 * self.BASE_POSITION_SIZE):
                    ...
                    if not opportunities:
                        ...
                        for pair_key, pair_info in opportunities:
                            # Calculate how many pairs we can afford (each pair is BASE_POSITION_SIZE tokens per leg)
                            ...
                            # Execute the trade (will always be 5 tokens per leg)
                    ...
                    print("⚠️  Insufficient USDC balance for new trades")
            else:
...
def main():
    """Main entry point"""
    print("\n" + "="*70)
    print("🎯 15-MINUTE BINARY OPTIONS PAIR TRADER - DYNAMIC EXIT")
    print("="*70)
