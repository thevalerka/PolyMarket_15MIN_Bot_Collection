...
class CorrelationPairTrader:
    ...
    # Trading parameters
    MAX_SUM_PRICE = 0.92  # Sum of asks must be less than this
    MIN_CORRELATION = 0.2 # Minimum correlation required
    MIN_TIME_TO_EXPIRY_MINUTES = 8  # Changed from 10 to 8
    MIN_VOLATILITY = 0.2  # Minimum volatility required

    # Exit thresholds
    STOP_LOSS_THRESHOLD = 0.40  # Sell when sum of bids < $0.50
    TAKE_PROFIT_THRESHOLD = 0.98  # Sell when sum of bids > $0.95

    # Position sizing
    POSITION_SIZE_PER_LEG = 10  # Fixed 5 tokens per leg
        ...
        for i, coin1 in enumerate(self.COINS):
            for coin2 in self.COINS[i+1:]:
                ...
                for direction in [(coin1, coin2), (coin2, coin1)]:
                    ...
                    # At least 1 coin is > 0.75
                    put_bid = pair_conditions['put_bid']
                    call_bid = pair_conditions['call_bid']

                    if put_bid < 0.74 and call_bid < 0.74 :
                        print("nessun Trending > 0.74")
                        continue
    ...
    def check_pair_conditions(self, coin1: str, coin2: str) -> Optional[Dict]:
        ...
        # Extract ask prices
        put_bid = put_data['best_bid'].get('price', 0)
        call_bid = call_data['best_bid'].get('price', 0)

        if put_ask == 0 or call_ask == 0:
            print("0$ PUT OR 0$ CALL in data")
            return None
    ...
    def execute_pair_trade(self, pair_info: Dict) -> bool:
        ...
        for wait_time in verification_times:
            ...
            if put_balance >= position_size * 0.98:
                put_confirmed = True
            ...
            if call_balance >= position_size * 0.98:
                call_confirmed = True
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                print(f"\n{'='*70}")
                print(f"🔄 Strategy D Conto39 CYCLE #{cycle_count} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                print(f"{'='*70}")
