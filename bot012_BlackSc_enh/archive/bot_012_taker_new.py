...
class BitcoinHourlyMarketBot:
    ...
    def cleanup_pending_orders_if_needed(self):
        """Check for and cancel any pending orders if enough time has passed"""
        current_time = time.time()

        if current_time - self.last_order_cleanup >= self.order_cleanup_interval:
            try:
                # Check for open orders
                self.track_api_call('get_open_orders_cleanup')
                open_orders = self.trader.get_open_orders()

                if open_orders and len(open_orders) > 0:
                    logger.warning(f"Found {len(open_orders)} pending orders - cancelling all (market strategy requires immediate execution)")
