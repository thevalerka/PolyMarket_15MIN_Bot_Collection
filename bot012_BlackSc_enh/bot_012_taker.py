...
class BitcoinHourlyMarketBot:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.no_trading_window_minutes = 2  # No trading for first 2 minutes of each hour
        self.is_in_no_trading_window = False  # Flag to indicate we're in no-trading period

        # Time condition threshold
        self.condition_threshold_seconds = 2.0  # 10 seconds
        ...
        logger.info(f"No-trading window: First {self.no_trading_window_minutes} minutes of each hour")
    ...
    def _load_config(self, config_path: str) -> Dict:
        ...
        default_config = {
            ...
            'price_offset': 0.04,           # Offset for market order conditions
            'max_spread': 0.06,             # Maximum acceptable bid-ask spread
            'min_usdc_balance': 4.0,        # Minimum USDC.e balance required
            'condition_threshold': 2.0,    # Seconds to wait before executing
    ...
    async def execute_hourly_reset(self):
        ...
        try:
            ...
            logger.warning("HOURLY RESET COMPLETED")
            logger.warning(f"NO-TRADING WINDOW: Trading suspended for next {self.no_trading_window_minutes} minutes")
    ...
    def execute_trading_strategy(self):
        ...
        try:
            ...
            if self.is_in_no_trading_period():
                now = datetime.now()
                time_until_trading = self.no_trading_window_minutes - now.minute
                
                # Update the no-trading flag and log occasionally
                if not self.is_in_no_trading_window:
                    self.is_in_no_trading_window = True
                    logger.warning(f"ENTERING NO-TRADING WINDOW: {now.strftime('%H:%M:%S')} - Trading suspended for {time_until_trading} more minutes")
                elif now.second % 30 == 0:  # Log every 30 seconds during no-trading period
                    logger.info(f"NO-TRADING WINDOW: {time_until_trading} minutes remaining until trading resumes")
                
                return
            else:
                # We're not in no-trading period, clear the flag if it was set
                if self.is_in_no_trading_window:
                    self.is_in_no_trading_window = False
                    now = datetime.now()
                    logger.warning(f"TRADING RESUMED: {now.strftime('%H:%M:%S')} - No-trading window ended")
        ...
        while True:
            try:
                ...
                if int(time.time()) % 30 == 0:
                    now = datetime.now()
                    no_trade_status = " [NO-TRADE WINDOW]" if self.is_in_no_trading_period() else ""
                    logger.info(f"Status [{now.strftime('%H:%M:%S')}]: USDC ${self.position.usdc_balance:.2f} | CALL: {self.position.call_balance:.1f}/{self.config['max_position_size']} | PUT: {self.position.put_balance:.1f}/{self.config['max_position_size']} | API calls: {self.api_call_stats['total_calls']}{no_trade_status}")
...
if __name__ == "__main__":
    ...
    print("• NEW: NO-TRADING WINDOW: Trading suspended first 2 minutes of each hour")
    print()
    print("CONFIGURATION:")
    ...
    print(f"• No-trading window: First 2 minutes of every hour (XX:00-XX:02)")
    print()
    print("CRITICAL - ORDER MANAGEMENT:")
    print("• ALL existing orders cancelled before every buy/sell attempt")
    print("• Orders that don't fill in 0.5 seconds trigger another cancel_all_orders()")
    print("• Periodic cleanup every 60 seconds cancels any remaining orders")
    print("• This ensures absolutely no stale orders can execute")
    print("• True market strategy: immediate execution only")
    print("• Multiple layers of order cancellation for maximum safety")
    print()
    ...
    print()
    print("HOURLY RESET + NO-TRADING WINDOW:")
    ...
    print("• THEN no-trading window:")
    print("  6. Suspend trading for first 2 minutes (XX:00-XX:02)")
    print("  7. Resume normal trading at XX:02")
    print()
    print("NEW NO-TRADING WINDOW DETAILS:")
    print("• Trading is COMPLETELY SUSPENDED during XX:00-XX:02 of every hour")
    print("• Bot will monitor market data but execute NO trades")
    print("• Price condition timers continue to run during this period")
    print("• Trading automatically resumes at XX:02")
    print("• This avoids trading during potential volatility at hour boundaries")
    print()
