#!/usr/bin/env python3
"""
Bitcoin Hourly Options Value Trading Bot

Value Trading Logic:
1. BUY: When ask price ≤ (probability / 2) AND probability ≥ 1.9%
2. SELL: When bid price ≥ (buy_price + 0.10)
3. Examples:
   - Probability 30% → Buy if ask ≤ 0.15, Sell if bid ≥ (buy_price + 0.10)
   - Probability 2% → Buy if ask ≤ 0.01, Sell if bid ≥ (buy_price + 0.10)
4. Fixed order size: 5 shares maximum
5. Refresh asset_id every hour
6. Track buy prices for profit target calculation
7. SAFETY: Cancel all orders before every trade
"""
...
class Position:
    ...
    call_buy_price: float = 0.0  # Price we paid for CALL tokens
    put_buy_price: float = 0.0   # Price we paid for PUT tokens
    call_purchased: bool = False
    put_purchased: bool = False

    # Track pending orders to prevent over-buying
    call_pending_buys: float = 0.0
    put_pending_buys: float = 0.0
...
class BitcoinHourlyValueBot:
    """Bitcoin hourly options value trading bot"""

    def __init__(self, config_path: str = None):
        """Initialize the value trading bot"""
        ...
        self.condition_threshold_seconds = 3.0  # 10 seconds

        logger.info("Bitcoin Hourly Options Value Trading Bot initialized")
        logger.info(f"Value trading logic: Buy when ask ≤ (probability/2) AND probability ≥ 1.9%")
        logger.info(f"                    Sell when bid ≥ (buy_price + $0.10)")
        ...
        logger.info(f"Minimum probability: {self.config['min_probability']:.1%}")
    ...
    def _load_config(self, config_path: str) -> Dict:
        ...
        default_config = {
            ...
            'min_probability': 0.015,       # Minimum probability to consider (1.9%)
            'profit_target': 0.10,          # Profit target above buy price ($0.10)
            'max_spread': 0.05,             # Maximum acceptable bid-ask spread
            'min_usdc_balance': 3.0,        # Minimum USDC.e balance required
            'condition_threshold': 3.0,    # Seconds to wait before executing
    ...
    async def execute_hourly_reset(self):
        ...
        try:
            ...
            # Reset buy prices for new contracts
            self.position.call_buy_price = 0.0
            self.position.put_buy_price = 0.0
    ...
    def execute_value_buy_order(self, token_name: str, asset_id: str, market_ask_price: float, current_balance: float) -> bool:
        """Execute a value buy order at current ask price"""
        try:
            ...
            cancelled_count = self.trader.cancel_all_orders()
            if cancelled_count > 0:
                logger.warning(f"Cancelled {cancelled_count} existing orders before {token_name} buy")
            ...
            if self.position.usdc_balance < total_cost:
                logger.info(f"Insufficient USDC for {token_name} value buy: ${self.position.usdc_balance:.2f} < ${total_cost:.2f}")
                return False
            ...
            logger.warning(f"EXECUTING VALUE BUY: {token_name} - {buy_size} shares @ ${market_ask_price:.4f}")
            logger.warning(f"  Current position: {fresh_balance:.1f} -> {fresh_balance + buy_size:.1f} shares (max: {self.config['max_position_size']})")

            # Execute buy by buying at the ask price
            ...
            if order_id:
                ...
                if order_status and order_status.get('status') not in ['FILLED', 'COMPLETED']:
                    ...
                    logger.warning(f"Order {order_id} not executed immediately (status: {order_status.get('status', 'unknown')}) - cancelling all orders")
                    ...
                    # Order executed successfully - record buy price for profit calculation
                    if token_name == 'CALL':
                        self.position.call_purchased = True
                        self.position.call_buy_price = market_ask_price  # Record purchase price
                        self.position.call_pending_buys = max(0, self.position.call_pending_buys - buy_size)
                    elif token_name == 'PUT':
                        self.position.put_purchased = True
                        self.position.put_buy_price = market_ask_price  # Record purchase price
                        self.position.put_pending_buys = max(0, self.position.put_pending_buys - buy_size)

                    logger.warning(f"VALUE BUY EXECUTED: {order_id} - {token_name} purchased @ ${market_ask_price:.4f}")
                    logger.info(f"Profit target: Sell when bid ≥ ${market_ask_price + self.config['profit_target']:.4f}")
                ...
                logger.error(f"Value buy FAILED for {token_name}")
                return False
            ...
            logger.error(f"Error executing value buy for {token_name}: {e}")
            return False

    def execute_value_sell_order(self, token_name: str, asset_id: str, market_bid_price: float, balance: float, buy_price: float) -> bool:
        """Execute a value sell order at current bid price"""
        try:
            # CRITICAL: Cancel all existing orders before placing new ones
            self.track_api_call(f'cancel_all_orders_before_sell_{token_name.lower()}')
            cancelled_count = self.trader.cancel_all_orders()
            if cancelled_count > 0:
                logger.warning(f"Cancelled {cancelled_count} existing orders before {token_name} sell")
            ...
            profit_target = buy_price + self.config['profit_target']

            logger.info(f"EXECUTING VALUE SELL: {token_name} - {sell_size:.2f} shares @ ${market_bid_price:.4f}")
            logger.info(f"  Bought @ ${buy_price:.4f}, Target ≥ ${profit_target:.4f}, Profit: ${market_bid_price - buy_price:.4f}")

            # Execute sell by selling at the bid price
            ...
            if order_id:
                ...
                if order_status and order_status.get('status') not in ['FILLED', 'COMPLETED']:
                    # Order didn't execute immediately - cancel everything again
                    logger.warning(f"Sell order {order_id} not executed immediately (status: {order_status.get('status', 'unknown')}) - cancelling all orders")
                    ...
                    profit = market_bid_price - buy_price
                    ...
                    logger.warning(f"VALUE SELL EXECUTED: {order_id} - {token_name} sold for ${profit:.4f} profit ({profit_pct:.1f}%)")

                    return True
            else:
                logger.warning(f"Value sell failed for {token_name}")
                return False

        except Exception as e:
            logger.error(f"Error executing value sell for {token_name}: {e}")
            return False

    def evaluate_value_conditions(self, prob_data: Dict, call_data: Dict, put_data: Dict):
        """Evaluate value trading conditions and update price condition tracking"""
        current_time = time.time()
        ...
        # Calculate value thresholds (buy when price ≤ probability/2)
        call_value_threshold = call_prob / 1.5
        put_value_threshold = put_prob / 1.5

        # CALL BUY CONDITION: ask ≤ (probability/2) AND probability ≥ min_probability AND not at position limit
        call_can_buy, _ = self.can_buy_more('CALL', self.position.call_balance)
        call_prob_ok = call_prob >= min_probability
        call_value_ok = call_ask <= call_value_threshold
        call_buy_condition_met = call_value_ok and call_prob_ok and call_can_buy
        self.position.call_buy_condition.update(call_buy_condition_met, current_time)

        # CALL SELL CONDITION: bid ≥ (buy_price + profit_target) AND we own the asset
        call_sell_target = self.position.call_buy_price + profit_target
        call_sell_condition_met = (call_bid >= call_sell_target and
                                  self.position.call_purchased and
                                  self.position.call_buy_price > 0)
        self.position.call_sell_condition.update(call_sell_condition_met, current_time)

        # PUT BUY CONDITION: ask ≤ (probability/2) AND probability ≥ min_probability AND not at position limit
        put_can_buy, _ = self.can_buy_more('PUT', self.position.put_balance)
        put_prob_ok = put_prob >= min_probability
        put_value_ok = put_ask <= put_value_threshold
        put_buy_condition_met = put_value_ok and put_prob_ok and put_can_buy
        self.position.put_buy_condition.update(put_buy_condition_met, current_time)

        # PUT SELL CONDITION: bid ≥ (buy_price + profit_target) AND we own the asset
        put_sell_target = self.position.put_buy_price + profit_target
        put_sell_condition_met = (put_bid >= put_sell_target and
                                 self.position.put_purchased and
                                 self.position.put_buy_price > 0)
        self.position.put_sell_condition.update(put_sell_condition_met, current_time)

        # Log conditions for debugging
        logger.debug(f"CALL: ask=${call_ask:.4f} ≤ threshold=${call_value_threshold:.4f}? {call_value_ok} | prob={call_prob:.1%} ≥ {min_probability:.1%}? {call_prob_ok} | can_buy? {call_can_buy} | BUY: {call_buy_condition_met} ({self.position.call_buy_condition.condition_duration:.1f}s)")
        logger.debug(f"CALL: bid=${call_bid:.4f} ≥ target=${call_sell_target:.4f}? {call_bid >= call_sell_target} | owned? {self.position.call_purchased} | SELL: {call_sell_condition_met} ({self.position.call_sell_condition.condition_duration:.1f}s)")
        logger.debug(f"PUT:  ask=${put_ask:.4f} ≤ threshold=${put_value_threshold:.4f}? {put_value_ok} | prob={put_prob:.1%} ≥ {min_probability:.1%}? {put_prob_ok} | can_buy? {put_can_buy} | BUY: {put_buy_condition_met} ({self.position.put_buy_condition.condition_duration:.1f}s)")
        logger.debug(f"PUT:  bid=${put_bid:.4f} ≥ target=${put_sell_target:.4f}? {put_bid >= put_sell_target} | owned? {self.position.put_purchased} | SELL: {put_sell_condition_met} ({self.position.put_sell_condition.condition_duration:.1f}s)")

    def execute_trading_strategy(self):
        """Execute the value trading strategy"""
        try:
            ...
            # Clean up any pending orders (value strategy requires immediate execution)
            ...
            # Evaluate value conditions and update timers
            self.evaluate_value_conditions(prob_data, call_data, put_data)
            ...
            if (self.position.call_buy_condition.condition_met and
                ...
                call_ask = float(call_data['best_ask']['price'])
                if self.execute_value_buy_order('CALL', self.current_call_asset_id, call_ask, self.position.call_balance):
            ...
            if (self.position.call_sell_condition.condition_met and
                ...
                call_bid = float(call_data['best_bid']['price'])
                if self.execute_value_sell_order('CALL', self.current_call_asset_id, call_bid,
                                                self.position.call_balance, self.position.call_buy_price):
            ...
            if (self.position.put_buy_condition.condition_met and
                ...
                put_ask = float(put_data['best_ask']['price'])
                if self.execute_value_buy_order('PUT', self.current_put_asset_id, put_ask, self.position.put_balance):
                    # Reset condition after successful purchase
                    self.position.put_buy_condition = PriceCondition()
            ...
            if (self.position.put_sell_condition.condition_met and
                ...
                put_bid = float(put_data['best_bid']['price'])
                if self.execute_value_sell_order('PUT', self.current_put_asset_id, put_bid,
                                               self.position.put_balance, self.position.put_buy_price):
                    # Reset condition after successful sale
                    self.position.put_sell_condition = PriceCondition()
            ...
            if self.position.call_buy_condition.condition_met or self.position.call_sell_condition.condition_met or self.position.put_buy_condition.condition_met or self.position.put_sell_condition.condition_met:
                logger.info(f"Probabilities - CALL: {call_prob:.1%} (threshold: {call_prob/2:.1%}), PUT: {put_prob:.1%} (threshold: {put_prob/2:.1%})")
                logger.info(f"Positions - CALL: {self.position.call_balance:.1f}@${self.position.call_buy_price:.4f} | PUT: {self.position.put_balance:.1f}@${self.position.put_buy_price:.4f}")

                if self.position.call_buy_condition.condition_met:
                    logger.info(f"CALL BUY value condition met for {self.position.call_buy_condition.condition_duration:.1f}s (need {threshold}s)")
                if self.position.call_sell_condition.condition_met:
                    call_sell_target = self.position.call_buy_price + self.config['profit_target']
                    logger.info(f"CALL SELL profit condition met for {self.position.call_sell_condition.condition_duration:.1f}s (target: ${call_sell_target:.4f})")
                if self.position.put_buy_condition.condition_met:
                    logger.info(f"PUT BUY value condition met for {self.position.put_buy_condition.condition_duration:.1f}s (need {threshold}s)")
                if self.position.put_sell_condition.condition_met:
                    put_sell_target = self.position.put_buy_price + self.config['profit_target']
                    logger.info(f"PUT SELL profit condition met for {self.position.put_sell_condition.condition_duration:.1f}s (target: ${put_sell_target:.4f})")
    ...
    async def run(self):
        """Main bot loop"""
        logger.info("Starting Bitcoin Hourly Options Value Trading Bot")
        logger.info(f"Order size: {self.config['order_size']} shares")
        logger.info(f"Min probability: {self.config['min_probability']:.1%}")
        logger.info(f"Profit target: ${self.config['profit_target']}")
        logger.info(f"Condition threshold: {self.condition_threshold_seconds}s")
        ...
        while True:
            try:
                ...
                if int(time.time()) % 30 == 0:
                    now = datetime.now()
                    call_status = f"${self.position.call_buy_price:.4f}" if self.position.call_purchased else "none"
                    put_status = f"${self.position.put_buy_price:.4f}" if self.position.put_purchased else "none"
                    logger.info(f" C40 ## Status [{now.strftime('%H:%M:%S')}]: USDC ${self.position.usdc_balance:.2f} | CALL: {self.position.call_balance:.1f}@{call_status} | PUT: {self.position.put_balance:.1f}@{put_status} | API: {self.api_call_stats['total_calls']}")

                # Wait before next cycle (faster for value conditions)
                await asyncio.sleep(0.1)  # Check every 0.1 seconds for real-time value monitoring
...
if __name__ == "__main__":
    print("BITCOIN HOURLY OPTIONS VALUE TRADING BOT")
    print("=" * 55)
    print("VALUE TRADING STRATEGY:")
    print("• BUY: When ask price ≤ (probability ÷ 2)")
    print("• SELL: When bid price ≥ (buy_price + $0.10)")
    print("• Example: If probability = 30% → Buy if ask ≤ $0.15")
    print("• Example: If bought @ $0.15 → Sell if bid ≥ $0.25")
    print("• Minimum probability filter: 1.9% (no buying below this)")
    print("• Fixed profit target: $0.10 per share")
    print("• 10-second condition persistence required")
    print("• HOURLY RESET: 30-second pause at 0:00 to reload asset IDs")
    print()
    print("CONFIGURATION:")
    print(f"• Order size: 5 shares per order")
    print(f"• Max position: 5 shares per asset")
    print(f"• Minimum probability: 1.9%")
    print(f"• Profit target: $0.10")
    print(f"• Condition threshold: 10 seconds")
    print(f"• Order execution: Value orders with immediate cancellation")
    print(f"• Order cleanup: Every 60 seconds")
    ...
    print()
    print("CRITICAL - VALUE STRATEGY:")
    print("• Only buys when getting 50%+ discount to fair value")
    print("• Sells for fixed $0.10 profit regardless of probability changes")
    print("• Tracks buy prices to calculate profit targets")
    print("• Ignores probabilities below 1.9% (too risky)")
    print("• All orders cancelled before every trade attempt")
    print("• Conservative value-based approach vs momentum trading")
    print()
    print("HOURLY RESET:")
    ...
    print("  5. Reset buy prices and purchase tracking")
    print("  6. Resume value trading")
    print()
