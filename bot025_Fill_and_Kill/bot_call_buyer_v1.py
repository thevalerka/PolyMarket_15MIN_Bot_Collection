...
class CallBuyerBot:
    def __init__(self):
        ...
        self.in_sell_mode = False  # When low on USDC, only sell, don't buy
    ...
    def place_call_order(self):
        ...
        try:
            # Place FAK order
            logger.info(f"📈 Placing CALL order: size={POSITION_SIZE}, price=${our_price:.2f} (bid=${best_bid_price:.2f}) = ${order_value:.2f}")
    ...
    def sell_call_position(self):
        """
        Sell CALL positions when we have them
        ...
        if self.call_positions_verified <= 0:
            logger.debug(f"SELL SKIP: No CALL positions (verified={self.call_positions_verified})")
            return
        ...
        try:
            logger.info(f"📉 Selling CALL: {positions_to_sell} position(s) = {quantity_to_sell} tokens @ ${our_price:.2f} (ask=${best_ask_price:.2f}) = ${order_value:.2f}")
    ...
    def print_status(self):
        ...
        if self.in_sell_mode:
            ...
            if self.call_positions_presumed >= MAX_CALL_POSITIONS:
                reasons.append(f"Max positions ({MAX_CALL_POSITIONS})")
            elif self.call_positions_presumed > self.put_positions_presumed:
                reasons.append(f"Imbalanced (C{self.call_positions_presumed}>P{self.put_positions_presumed})")
            elif self.call_positions_verified > self.put_positions_verified:
                reasons.append(f"Verified imbalance (C{self.call_positions_verified}>P{self.put_positions_verified})")
