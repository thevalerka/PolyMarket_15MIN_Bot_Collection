#!/usr/bin/env python3
"""
Strategy D32 - Pure EMA Crossover Trading Bot
...
- Entry at $0.50, current at $0.45 → |0.45 - 0.50| = $0.05 > $0.04 → Close ✅
 ...
 pm2 start bot_d3_2.py --cron-restart="00 * * * *" --interpreter python3
"""
...
# Persistence
STATE_FILE = "/home/ubuntu/013_2025_polymarket/bot017_RSI/bot_d32_state.json"
...
def load_trades():
    ...
    trades_file = os.path.join(TRADES_DIR, f"trades_D32_{today}.json")
    ...
    while True:
        try:
            ...
            if int(current_time) % 10 == 0:
                ...
                last_str = last_d32_type_opened if last_d32_type_opened else "None"
                ...
                if ema is not None and upper_band is not None:
                    ...
                    print(f"[D32 {datetime.fromtimestamp(timestamp, tz=timezone.utc).strftime('%H:%M:%S')}] "
            ...
            if position and position['type'] == 'CALL' and rsi > RSI_EXIT_CALL:
                if call_bid is not None:
                    price_movement = (call_bid - position['entry_price'])
                    if price_movement > MIN_PRICE_MOVEMENT_TP or price_movement < MIN_PRICE_MOVEMENT_SL:
                        signal = 'CLOSE_CALL'
                        print(f"\n[SIGNAL] RSI EXIT | RSI={rsi:.1f} > {RSI_EXIT_CALL} | Price moved ${price_movement:.3f}")
                    else:
                        print(f"[D32] RSI exit signal but price movement too small: ${price_movement:.3f} ")

            elif position and position['type'] == 'PUT' and rsi < RSI_EXIT_PUT:
                if put_bid is not None:
                    price_movement = put_bid - position['entry_price']
                    if price_movement > MIN_PRICE_MOVEMENT_TP or price_movement < MIN_PRICE_MOVEMENT_SL:
                        signal = 'CLOSE_PUT'
            ...
            # Priority 2: Flip wrong position (D3: only if option price moved > $0.04)
                ...
                if call_bid is not None:
                    ...
                    if price_movement > MIN_PRICE_MOVEMENT_TP or price_movement < MIN_PRICE_MOVEMENT_SL:
                        ...
                        print(f"[D32] Flip signal but price movement too small: ${price_movement:.3f} ")
