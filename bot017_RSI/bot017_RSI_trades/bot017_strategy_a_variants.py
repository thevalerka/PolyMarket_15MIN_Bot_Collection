#!/usr/bin/env python3
"""
RSI Trading Bot - STRATEGIES A, A2, C, C2, C3
...
STRATEGY C2: Adaptive RSI Period with Fixed TP/SL
...
- ATR: 6 periods × 15 seconds, exponentially weighted
...
class ATRCalculator:
    """Calculate Average True Range with exponential weighting for adaptive TP/SL"""
...
def calculate_dynamic_tpsl_c3(atr: float, entry_price: float, atr_calculator: ATRCalculator) -> Tuple[float, float]:
    """
    Calculate dynamic TP and SL for Strategy C3 using ATR
    Maintains 1:3 ratio (TP:SL)
    """
    prob_movement, expected_magnitude = atr_calculator.estimate_movement_probability(atr, entry_price)
    
    # Base TP/SL with 1:3 ratio
    base_tp = ATR_BASE_TP_C3
    base_sl = ATR_BASE_SL_C3  # 3× of TP
    
    scaling_factor = expected_magnitude * prob_movement
    
    tp_distance = base_tp + (scaling_factor * 1.0)
    sl_distance = base_sl + (scaling_factor * ATR_TP_SL_RATIO_C3)
    
    # Clamp and maintain ratio
    tp_distance = max(0.015, min(0.20, tp_distance))
    sl_distance = tp_distance * ATR_TP_SL_RATIO_C3  # Maintain 1:3 ratio
    
    take_profit = entry_price + tp_distance
    stop_loss = sl_distance  # Return as absolute value for check_tp_sl
...
def main():
    print("\n" + "="*150)
    print("RSI TRADING BOT - STRATEGIES A, A2, C, C2, C3")
    ...
    print(f"C2: Adaptive RSI Period + Fixed TP/SL ({RSI_LOWER_C2}/{RSI_UPPER_C2}, 0-3/4-7/8+, TP:${TAKE_PROFIT_C2} SL:${STOP_LOSS_C2})")
    print(f"C3: Adaptive RSI Period + ATR TP/SL ({RSI_LOWER_C3}/{RSI_UPPER_C3}, 0-3/4-7/8+, 1:3 ratio)")
    ...
    position_a, position_a2, position_c, position_c2, position_c3 = None, None, None, None, None
    ...
    # Strategy C2 state (adaptive RSI period with fixed TP/SL)
    ...
    breach_history_c3 = deque(maxlen=1000)
    ...
    # ATR tracking for C3
    atr_calculator_c3 = ATRCalculator()
    atr_price_history_c3 = deque(maxlen=100)  # 100 seconds for 6x15s periods
    atr_timestamp_history_c3 = deque(maxlen=100)
    ...
    try:
        while True:
            ...
            if is_period_start and now.second == 0:
                ...
                if position_a or position_a2 or position_c or position_c2 or position_c3:
                    time.sleep(2)
            ...
            # Update ATR price history for C3
            atr_price_history_c3.append(btc_price)
            atr_timestamp_history_c3.append(datetime.now())
            ...
            if current_time_threshold - last_threshold_adjustment_time >= threshold_check_interval:
                ...
                rsi_period_c3, total_breaches_c3 = adjust_rsi_period(
                    ...
                    RSI_PERIOD_HIGH_TRIGGER_C3, RSI_PERIOD_LOW_TRIGGER_C3, RSI_PERIOD_MIN_C3)
                rsi_period_samples_c3 = rsi_period_c3 * 10
            ...
            atr_c3 = atr_calculator_c3.calculate_atr(
                list(atr_price_history_c3),
                list(atr_timestamp_history_c3),
                ATR_PERIOD_SECONDS_C3,
                ATR_NUM_PERIODS_C3
            ) if len(atr_price_history_c3) >= 20 else None
            atr_c3_str = f"{atr_c3:.4f}" if atr_c3 is not None else "N/A"
            
            # Split display across 2 lines for readability
            print(f"[{timestamp}] BTC:${btc_price:.2f} | A:[{rsi_lower_a:.0f}/{rsi_upper_a:.0f}] {pos_a_str:<12} ${trades_a['total_pnl']:+.2f} | A2:[{rsi_lower_a2:.0f}/{rsi_upper_a2:.0f}] {pos_a2_str:<12} ${trades_a2['total_pnl']:+.2f}")
            print(f"         C:RSI{rsi_period_c}s={rsi_c_str} {pos_c_str:<12} ${trades_c['total_pnl']:+.2f} | C2:RSI{rsi_period_c2}s={rsi_c2_str} {pos_c2_str:<12} ${trades_c2['total_pnl']:+.2f} | C3:RSI{rsi_period_c3}s={rsi_c3_str} ATR:{atr_c3_str} {pos_c3_str:<12} ${trades_c3['total_pnl']:+.2f}", end='\r')
            ...
            # Check TP/SL for C2 (fixed TP/SL)
            if position_c2:
            ...
            if position_c3:
                ...
                if atr_c3_current:
                    tp_target, sl_distance = calculate_dynamic_tpsl_c3(atr_c3_current, position_c3['entry_price'], atr_calculator_c3)
                    current_bid_c3 = put_bid if position_c3['type'] == 'PUT' else call_bid
                    current_pnl = current_bid_c3 - position_c3['entry_price']
                    
                    # Debug output every 10 seconds
                    if int(time.time()) % 10 == 0:
                        print(f"\n[C3 DEBUG] Entry:${position_c3['entry_price']:.3f} Current:${current_bid_c3:.3f} PNL:${current_pnl:+.3f} | ATR:{atr_c3_current:.4f} TP:${tp_target:.3f}(+${tp_target-position_c3['entry_price']:.3f}) SL:${sl_distance:.3f}")
        ...
        # Print summaries
        for strat, data in [("A", trades_a), ("A2", trades_a2), ("C", trades_c), ("C2", trades_c2), ("C3", trades_c3)]:
