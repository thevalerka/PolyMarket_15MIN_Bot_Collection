#!/usr/bin/env python3
"""
Strategy D2 - EMA Bands Trading Bot
...
# Persistence
STATE_FILE = "/home/ubuntu/013_2025_polymarket/bot017_RSI/bot_d2_state.json"
...
def load_trades():
    ...
    trades_file = os.path.join(TRADES_DIR, f"trades_D2_{today}.json")
...
def main():
    ...
    while True:
        try:
            ...
            # Calculate tolerance (can be done every loop, uses volatility from last sample)
            ...
            if int(current_time) % 10 == 0:
                ...
                if ema is not None and upper_band is not None:
                    ...
                    print(f"[D2 {datetime.fromtimestamp(timestamp, tz=timezone.utc).strftime('%H:%M:%S')}] "
                          f"BTC:${btc_price:.2f} {btc_vs_bands} [${lower_band:.2f} EMA:${ema_str} ${upper_band:.2f}] "
                          f"Tol:${tolerance:.2f} Vol:${volatility:.2f} RSI{rsi_period}s={rsi_str} "
