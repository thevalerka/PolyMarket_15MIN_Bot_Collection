...
class OrderBookFeatureExtractor:
    ...
    def extract_features(orderbook: Dict, top_n: int = TOP_N_LEVELS) -> Optional[np.ndarray]:
        ...
        IMPORTANT: We check if MID price is in range for TRAINING purposes,
        but we still use the ACTUAL best_bid/best_ask prices (even if extreme)
        """
        if not orderbook or 'complete_book' not in orderbook:
            return None

        # Get ACTUAL mid price (using real best_bid/best_ask, even if extreme)
        mid_price = OrderBookFeatureExtractor.get_mid_price(orderbook)

        # Skip training on extreme markets (but still use real prices)
        ...
        # 3. Aggregate features (using real mid_price)
    ...
    def get_mid_price(orderbook: Dict) -> Optional[float]:
        """Calculate mid price, handling NULL bid/ask at extremes"""
        best_bid = orderbook.get('best_bid')
        best_ask = orderbook.get('best_ask')
        
        # Both exist and have prices
        if best_bid and best_ask and best_bid.get('price') and best_ask.get('price'):
            return (best_bid['price'] + best_ask['price']) / 2
        
        # Only bid exists (deep ITM for CALL, use bid as proxy)
        if best_bid and best_bid.get('price'):
            return best_bid['price']
        
        # Only ask exists (deep OTM for CALL, use ask as proxy)
        if best_ask and best_ask.get('price'):
            return best_ask['price']
        
        # Neither exists (shouldn't happen, but handle it)
        return None
...
class OrderBookPredictor:
    ...
    def get_current_prediction(self) -> Optional[Dict]:
        """Get prediction from current buffer with trading signal"""
        ...
        # Get current orderbook for actual bid/ask prices
        current_orderbook = self.snapshot_buffer[-1]['orderbook']
        best_bid = current_orderbook.get('best_bid', {})
        best_ask = current_orderbook.get('best_ask', {})

        bid_price = best_bid.get('price') if best_bid else None
        ask_price = best_ask.get('price') if best_ask else None
        mid_price = self.snapshot_buffer[-1]['mid_price']

        # Determine signal based on new thresholds
        if prediction >= 0.75:
            signal = 'BUY'
            trade_price = ask_price if ask_price else mid_price
        elif prediction <= 0.25:
            signal = 'SELL'
            trade_price = bid_price if bid_price else mid_price
        else:
            signal = 'HOLD'
            trade_price = None
