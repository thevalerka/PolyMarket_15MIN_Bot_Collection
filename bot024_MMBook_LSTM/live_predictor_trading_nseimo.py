                        ...
                        if prediction:
                            ...
                            if self.trade_tracker.current_position is None:
                                ...
                                if self.iteration % 20 == 0:
                                    ...
                                    # Check if bid/ask is NULL
                                    bid_price = prediction.get('bid_price')
                                    ask_price = prediction.get('ask_price')
                                    
                                    if signal == 'BUY':
                                        if ask_price is None:
                                            print(f"     ⚠️  Ask is NULL (extreme ITM/OTM) - NO TRADE")
                                        elif trade_price:
                                            print(f"     Would BUY @ ${trade_price:.4f}")
                                    elif signal == 'SELL':
                                        if bid_price is None:
                                            print(f"     ⚠️  Bid is NULL (extreme ITM/OTM) - NO TRADE")
                                        elif trade_price:
                                            print(f"     Would SELL @ ${trade_price:.4f}")
