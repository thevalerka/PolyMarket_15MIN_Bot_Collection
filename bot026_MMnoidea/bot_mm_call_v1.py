#!/usr/bin/env python3
"""
CALL Market Maker Bot - Provides liquidity with limit orders
=============================================================
...
pm2 start bot_mm_call_v1.py --cron-restart="00 * * * *" --interpreter python3
"""
...
# Files
CREDENTIALS_FILE = env_path = '/home/ubuntu/013_2025_polymarket/keys/keys_ovh38.env'  # CALL bot wallet
CALL_FILE = "/home/ubuntu/013_2025_polymarket/15M_CALL.json"
BALANCE_FILE = "/home/ubuntu/013_2025_polymarket/bot025_Fill_and_Kill/call_mm_status.json"

# Trading parameters
POSITION_SIZE = 5  # Tokens per order
...
class CallMarketMakerBot:
    def __init__(self):
        logger.info("🚀 Initializing CALL Market Maker Bot...")
    ...
    def update_positions_and_balance(self):
        ...
        if self.current_call_id:
            balance = self.check_token_balance(self.current_call_id)
    ...
    def load_asset_ids(self) -> bool:
        """Load CALL asset ID (rate limited)"""
        ...
        if not call_data:
            logger.error("❌ Could not load CALL asset ID")
            return False

        new_call_id = call_data.get('asset_id')
        if not new_call_id:
            logger.error("❌ CALL asset ID not found")
            return False

        # Check if new period
        if new_call_id != self.current_call_id:
    ...
    def get_call_orderbook(self) -> Optional[dict]:
        """Get CALL orderbook from JSON file"""
        try:
            logger.debug(f"Reading orderbook from: {CALL_FILE}")
            call_data = read_json_safe(CALL_FILE)
            if not call_data:
                logger.warning(f"❌ read_json_safe returned None for {CALL_FILE}")
                return None

            logger.debug(f"✅ Loaded orderbook - best_bid: {call_data.get('best_bid')}, best_ask: {call_data.get('best_ask')}")
            # The JSON file already has best_bid and best_ask
            return call_data
        except Exception as e:
    ...
    def cancel_all_open_orders(self):
        """Cancel ALL open orders from API (not just tracked ones)"""
        try:
            ...
            for order in open_orders:
                ...
                if order_id:
                    logger.info(f"🗑️  Cancelling open order: {order_id[:8]}...")
                    self.trader.cancel_all_orders()
    ...
    def place_buy_limit(self, price: float, quantity: int) -> Optional[str]:
        ...
        if self.usdc_balance < (quantity * price):
            logger.warning(f"⚠️  Insufficient USDC: ${self.usdc_balance:.2f} < ${quantity * price:.2f}")
            return None
    ...
    def manage_orders(self):
        ...
        # Get orderbook
        orderbook = self.get_call_orderbook()
        if not orderbook:
        ...
        if not best_bid or not isinstance(best_bid, dict):
            logger.warning("⚠️  No valid best_bid in orderbook")
            return
        if not best_ask or not isinstance(best_ask, dict):
            logger.warning("⚠️  No valid best_ask in orderbook")
            return
        ...
        if not best_bid_price or not best_ask_price:
            logger.warning(f"⚠️  Missing prices - bid: {best_bid_price}, ask: {best_ask_price}")
            return

        # Check spread
        spread = best_ask_price - best_bid_price
        if spread > MAX_SPREAD:
            logger.warning(f"⚠️  Spread too wide: ${spread:.2f} > ${MAX_SPREAD}")
            return
        ...
        logger.debug(f"Market: Bid=${best_bid_price:.2f}, Ask=${best_ask_price:.2f}, Spread=${spread:.2f}")
        logger.debug(f"Our prices: Buy=${our_buy_price:.2f}, Sell=${our_sell_price:.2f}")
        ...
        if not in_last_minute:  # Only place buys outside last minute
            ...
            if not self.active_buy_order_id:
                ...
                logger.debug("No active buy order - will place new one")
            elif self.buy_order_price and abs(self.buy_order_price - our_buy_price) > 0.01:
                # Price moved significantly, cancel and replace
                logger.info(f"Price moved: ${self.buy_order_price:.2f} -> ${our_buy_price:.2f}, replacing order")
                self.trader.cancel_all_orders()
                self.active_buy_order_id = None
                should_update_buy = True
            else:
                logger.debug(f"BUY order active @ ${self.buy_order_price:.2f} (current: ${our_buy_price:.2f})")

            # Place buy order (only if we don't have one active)
            if should_update_buy:
                # Check token balance - don't buy if we already have > POSITION_SIZE tokens
                current_token_balance = self.check_token_balance(self.current_call_id)
                if current_token_balance >= POSITION_SIZE:
                    logger.warning(f"⚠️  Already have {current_token_balance:.2f} tokens (>= {POSITION_SIZE}), not buying")
                    should_update_buy = False

                # Check USDC balance - need enough for the order
                order_value = POSITION_SIZE * our_buy_price
                if self.usdc_balance < order_value:
                    logger.warning(f"⚠️  Insufficient USDC: ${self.usdc_balance:.2f} < ${order_value:.2f} needed")
                    should_update_buy = False

                # Check position limit
                if self.long_positions >= MAX_LONG_POSITIONS:
                    logger.debug(f"At max positions: {self.long_positions}/{MAX_LONG_POSITIONS}")
                    should_update_buy = False

                # Check minimum order value
                if order_value < MIN_ORDER_VALUE:
                    logger.debug(f"Order value too low: ${order_value:.2f} < ${MIN_ORDER_VALUE}")
                    should_update_buy = False
        ...
        if self.long_positions > 0:
            ...
            if should_update_sell:
                # Get actual balance and floor to avoid floating point issues
                actual_balance = self.check_token_balance(self.current_call_id)
                # Use floor division to get whole positions, avoiding floating point errors
                ...
                if quantity_to_sell >= POSITION_SIZE:
                    ...
                    if order_value >= MIN_ORDER_VALUE:
                        ...
                        order_id = self.place_sell_limit(our_sell_price, int(quantity_to_sell))
                        if order_id:
    ...
    def print_status(self):
        ...
        logger.info("=" * 70)
        logger.info(f"{status_icon} CALL MM STATUS: {mode}")
    ...
    def run(self):
        """Main bot loop"""
        logger.info("=" * 70)
        logger.info("🤖 CALL MARKET MAKER BOT STARTED")
        logger.info("=" * 70)
        ...
        logger.info("🗑️  Cancelling all existing orders...")
