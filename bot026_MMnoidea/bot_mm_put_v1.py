...
class PutMarketMakerBot:
    ...
    def get_put_orderbook(self) -> Optional[dict]:
        """Get PUT orderbook from JSON file"""
        try:
            logger.debug(f"Reading orderbook from: {PUT_FILE}")
            put_data = read_json_safe(PUT_FILE)
            if not put_data:
                logger.warning(f"❌ read_json_safe returned None for {PUT_FILE}")
                return None

            logger.debug(f"✅ Loaded orderbook - best_bid: {put_data.get('best_bid')}, best_ask: {put_data.get('best_ask')}")
    ...
    def manage_orders(self):
        ...
        if not in_last_minute:  # Only place buys outside last minute
            ...
            if should_update_buy:
                ...
                current_token_balance = self.check_token_balance(self.current_put_id)
