#!/usr/bin/env python3
"""
15-Minute Binary Options Order Book Imbalance Trading Bot - PRODUCTION v3
==========================================================================
Strategy: Order Book Imbalance - HOLD UNTIL EXPIRATION (No Sells)

Trading Rules:
- BUY when: sum(top4_bids) > sum(top4_asks) × IMBALANCE_RATIO
- NO SELLS - Hold all positions until period expiration (saves commissions)
- NO BUY if: price > 0.90 OR price < 0.10

Position Rules (enforced based on WALLET-CONFIRMED fills):
...
- ALTERNATING: When balanced, must alternate CALL ↔ PUT
  - Tracked by detecting which side INCREASED in wallet (confirmed fill)
...
- Orders cancelled after timeout if not filled
- last_buy_side only updated when wallet shows fill

pm2 start usdc_monitor39and40.py --cron-restart="00 */6 * * *"  --interpreter python3
"""
...
MAX_POSITIONS_PER_SIDE = 5  # Max 1 CALL and 1 PUT at a time
...
class OrderBookImbalanceBot:
    ...
    def __init__(self, credentials: dict):
        ...
        logger.info("="*70)
        logger.info("🤖 ORDER BOOK IMBALANCE BOT - PRODUCTION v3")
        logger.info("="*70)
        ...
        logger.info(f"ALTERNATING: Must alternate CALL ↔ PUT (wallet-verified)")
    ...
    def can_buy_side(self, side: str) -> Tuple[bool, str]:
        ...
        2. ALTERNATING rule: Must alternate based on last FILLED order

        Rules:
        - If CALL > PUT: Can ONLY buy PUT (to balance)
        - If PUT > CALL: Can ONLY buy CALL (to balance)
        - If CALL == PUT and last_buy_side is set: Must alternate
        - If CALL == PUT and last_buy_side is None: First buy, can buy either
        """
        ...
        # ALTERNATING rule (only applies when positions are balanced)
    ...
    def display_status(self, call_analysis: Optional[Dict], put_analysis: Optional[Dict]):
        ...
        # Show what can be bought next based on EVEN + ALTERNATING rules
        ...
        while True:
            try:
                ...
                if call_buy_signal and call_analysis:
                    # Check price limits and timing
                    can_order, order_reason = self.can_place_order(call_analysis['best_ask'])
                    if can_order:
                        # Check EVEN + ALTERNATING rules (based on wallet-confirmed fills)
