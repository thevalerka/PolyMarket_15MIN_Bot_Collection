...
# Persistence
STATE_FILE = "sim_last_minute_eth_state.json"
TRADES_DIR = "sim_last_minute_eth_trades"
...
def fetch_period_candle(period_start: datetime) -> Optional[Dict]:
    ...
    try:
        ...
        params = {
            'symbol': 'ETHUSDT',
            'interval': '15m',
            'startTime': int(period_start.timestamp() * 1000),
            'limit': 1
        }
        ...
        print(f"[ERROR] Failed to fetch candle from Binance: {e}")
        return None
...
# File paths
PUT_FILE = "/home/ubuntu/013_2025_polymarket/15M_PUT_ETH.json"
CALL_FILE = "/home/ubuntu/013_2025_polymarket/15M_CALL_ETH.json"
ETH_FILE = "/home/ubuntu/013_2025_polymarket/bybit_eth_price.json"

def get_live_eth_price() -> Optional[float]:
    """Get ETH price from local file"""
    try:
...
def get_live_option_prices() -> Tuple[Optional[float], Optional[float], Optional[float], Optional[float]]:
    """
    Get CALL and PUT prices from local files with validation
    ...
    # Read CALL file with retry and validation
    for attempt in range(3):
        try:
            with open(CALL_FILE, 'r') as f:
                call_data = json.load(f)

            # Validate it's actually CALL data
            if call_data.get('asset_name') == '15M_CALL':
                ...
                # Wrong data, wait and retry
                time.sleep(0.05)
        except:
            time.sleep(0.05)

    # Read PUT file with retry and validation
    for attempt in range(3):
        try:
            with open(PUT_FILE, 'r') as f:
                put_data = json.load(f)

            # Validate it's actually PUT data
            if put_data.get('asset_name') == '15M_PUT':
...
def main():
    ...
    print("=" * 80)
    print("LAST-MINUTE VOLATILITY SCALPING SIMULATOR - ETHEREUM")
    print("=" * 80)
    ...
    try:
        while True:
            ...
            # Update tracker
            tracker.update(timestamp, eth_price)
            ...
            # Calculate distance from strike
            distance = eth_price - strike_price
            ...
            if should_log:
                ...
                # SANITY CHECK: Detect if data looks swapped
                # When ETH > strike: CALL should be expensive, PUT cheap
                # When ETH < strike: PUT should be expensive, CALL cheap
                data_warning = ""
                if call_bid and put_bid:
                    if distance > 10:  # Significantly above strike (ETH moves less than BTC)
                        if put_bid > call_bid:
                            data_warning = " ⚠️DATA_SWAP?"
                    elif distance < -10:  # Significantly below strike
                        if call_bid > put_bid:
                            data_warning = " ⚠️DATA_SWAP?"
                ...
                print(f"{prefix}[{time_str}] ETH:${eth_price:.2f} | Dist:${distance:+.2f} | "
                      f"Vol-L:{vol_long_str} Vol-S:{vol_short_str} | Chop:{choppiness:.1f} | "
                      f"C:{call_bid_str}/{call_ask_str} P:{put_bid_str}/{put_ask_str}{data_warning} | "
                      f"TTL:{ttl_str}{pos_info}")
            ...
            if time_to_expiry <= ENTRY_WINDOW_LONG and time_to_expiry > 0 and position is None:
                ...
                if signal and balance >= POSITION_SIZE:
                    ...
                    print(f"  Time: {timestamp.strftime('%H:%M:%S')}")
                    print(f"  Reason: {signal['reason']}")
                    print(f"  ETH: ${eth_price:.2f} (Distance: ${distance:+.2f} from strike)")
                    print(f"  Vol-Long: ${vol_long:.2f} | Vol-Short: ${vol_short:.2f}")
                    print(f"  Choppiness: {choppiness:.1f}")
                    print(f"  Time to Expiry: {time_to_expiry:.0f}s")
                    print(f"  Balance: ${balance:.2f}")
        ...
        if position is not None:
            print(f"\n{'='*80}")
            print(f"⏰ EXPIRY - Fetching 15-min candle from Binance API...")
            print(f"{'='*80}")

            candle = fetch_period_candle(period_start)

            if candle:
                final_eth = candle['close']
                ...
                print(f"\n  Settlement:")
                print(f"    Strike: ${strike_price:.2f}")
                print(f"    Close:  ${final_eth:.2f}")

                # Determine final value
                if position['type'] == 'CALL':
                    final_value = 1.0 if final_eth > strike_price else 0.0
                    result_text = "WIN ✅" if final_eth > strike_price else "LOSS ❌"
                    print(f"    Type: CALL (win if close > strike)")
                    print(f"    Result: {result_text}")
                else:  # PUT
                    final_value = 1.0 if final_eth < strike_price else 0.0
