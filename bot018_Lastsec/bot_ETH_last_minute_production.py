...
pm2 start bot_ETH_last_minute_production.py --cron-restart="00 * * * *" --interpreter python3
"""
...
STATE_FILE = "/home/ubuntu/013_2025_polymarket/bot017_RSI/bot_ETH_last_minute_state.json"
TRADES_DIR = "/home/ubuntu/013_2025_polymarket/bot017_RSI/bot_ETH_last_minute_trades"

# Trading Parameters
POSITION_SIZE = 5.0  # 5 shares
MIN_BUY_PRICE = 0.55  # Minimum price to open position
...
def get_eth_price():
    """Get current ETH price from local JSON file"""
    try:
        data = read_json(ETH_FILE)
        if data:
...
class BotLastMinute:
    ...
    def __init__(self, credentials: dict):
        ...
        self.last_eth_price: Optional[float] = None
    ...
    def execute_buy(self, token_type: str, token_id: str, ask_price: float,
                    eth_price: float, strike_price: float, distance: float,
    ...
    def close_position_at_expiry(self, candle: Dict):
        ...
        try:
            ...
            logger.info(f"  Candle Close: ${final_eth:.2f}")
            logger.info(f"  Strike: ${strike:.2f}")

            # Determine final value
            if self.position.token_type == 'CALL':
                final_value = 1.0 if final_eth > strike else 0.0
                result_text = "WIN ✅" if final_eth > strike else "LOSS ❌"
            ...
            trade = {
                ...
                'entry_eth': self.position.entry_eth_price,
                'exit_eth': final_eth,
                'strike_price': strike,
    ...
    def determine_entry_signal(self, choppiness: float, distance: float, vol_long: float, vol_short: float,
        ...
        if abs(distance) <= vol_threshold:
            print("TooClose - Period ", window_name)
            return None

        # Choppiness filter: only trade when market is trending (choppiness <= threshold)
        if choppiness > MAX_CHOPPINESS:
            print("toochoppy")
            logger.debug(f"❌ Market too choppy: {choppiness:.1f} > {MAX_CHOPPINESS}")
            return None
            ...
            while True:
                ...
                if period_start_min != self.current_period_start:
                    ...
                    if self.strike_price :
                        print("CURRENT STRIKE:",self.strike_price)
                ...
                # Update tracker
                self.tracker.update(now, eth_price)
                ...
                distance = eth_price - self.strike_price
                ...
                print(f"{prefix}[{now.strftime('%H:%M:%S')}] ETH:${eth_price:.2f} | Dist:${distance:+.2f} | "
                ...
                if (seconds_remaining <= ENTRY_WINDOW_LONG and
                    ...
                    if signal and self.can_open_position():
                        ...
                        self.execute_buy(
                            ...
                            eth_price, self.strike_price, distance,
