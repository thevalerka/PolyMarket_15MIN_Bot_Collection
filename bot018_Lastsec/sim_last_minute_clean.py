    ...
    try:
        while True:
            ...
            if should_log:
                ...
                # When BTC > strike: CALL should be expensive, PUT cheap
                # When BTC < strike: PUT should be expensive, CALL cheap
                data_warning = ""
                if call_bid and put_bid:
                    if distance > 100:  # Significantly above strike
                        if put_bid > call_bid:
                            data_warning = " ⚠️DATA_SWAP?"
                    elif distance < -100:  # Significantly below strike
                        if call_bid > put_bid:
                            data_warning = " ⚠️DATA_SWAP?"
