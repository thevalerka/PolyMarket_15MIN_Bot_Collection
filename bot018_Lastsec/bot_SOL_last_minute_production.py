...
pm2 start bot_SOL_last_minute_production.py --cron-restart="00 * * * *" --interpreter python3
"""
...
# File paths
PUT_FILE = "/home/ubuntu/013_2025_polymarket/15M_PUT_SOL.json"
CALL_FILE = "/home/ubuntu/013_2025_polymarket/15M_CALL_SOL.json"
SOL_FILE = "/home/ubuntu/013_2025_polymarket/bybit_sol_price.json"
STATE_FILE = "/home/ubuntu/013_2025_polymarket/bot017_RSI/bot_SOL_last_minute_state.json"
TRADES_DIR = "/home/ubuntu/013_2025_polymarket/bot017_RSI/bot_SOL_last_minute_trades"
...
def get_sol_price():
    """Get current SOL price from local JSON file"""
    try:
        data = read_json(SOL_FILE)
        if data:
...
class BotLastMinute:
    ...
    def execute_buy(self, token_type: str, token_id: str, ask_price: float,
                    sol_price: float, strike_price: float, distance: float,
    ...
    def close_position_at_expiry(self, candle: Dict):
        ...
        try:
            ...
            final_sol = candle['close']
            strike = self.position.strike_price

            logger.info(f"  Candle Close: ${final_sol:.2f}")
            logger.info(f"  Strike: ${strike:.2f}")

            # Determine final value
            if self.position.token_type == 'CALL':
                final_value = 1.0 if final_sol > strike else 0.0
                result_text = "WIN ✅" if final_sol > strike else "LOSS ❌"
            ...
            trade = {
                ...
                'entry_sol': self.position.entry_sol_price,
                'exit_sol': final_sol,
                'strike_price': strike,
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Update tracker
                self.tracker.update(now, sol_price)
                ...
                distance = sol_price - self.strike_price
                ...
                print(f"{prefix}[{now.strftime('%H:%M:%S')}] SOL:${sol_price:.2f} | Dist:${distance:+.2f} | "
                ...
                if (seconds_remaining <= ENTRY_WINDOW_LONG and
                    ...
                    if signal and self.can_open_position():
                        ...
                        self.execute_buy(
                            ...
                            sol_price, self.strike_price, distance,
