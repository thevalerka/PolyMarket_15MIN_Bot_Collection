...
class Enhanced15MinBinaryOptionCalculator:
    def __init__(self, price_file_path="/home/ubuntu/013_2025_polymarket/btc_price.json"):
        ...
        self.results_base_path = "/home/ubuntu/013_2025_polymarket/bot018_Lastsec/BS_data/"
    ...
    def _initialize_daily_file(self):
        """Initialize results file path (single file, overwritten every second)"""
        # Create directory if it doesn't exist
        os.makedirs(self.results_base_path, exist_ok=True)

        # Use a fixed filename that will be overwritten
        self.results_file = f"{self.results_base_path}last_option_data.json"
        
        print(f"📂 Using file: {self.results_file}")
    ...
    def save_results_to_file(self, result):
        """Save probability results immediately to single JSON file (overwrite mode)"""
        try:
            # Write result immediately, overwriting the file
    ...
    def run_continuous(self):
        ...
        print("💡 Data overwrites single file every second (last info only)")
...
def force_refresh_data():
    ...
    print(f"💾 Data folder: {calc.results_base_path}")
    print(f"📊 Data file: {calc.get_daily_filename()}")
...
def show_daily_stats():
    ...
    if os.path.exists(calc.results_file):
        ...
        print(f"📊 No data file exists yet")
...
def show_refresh_schedule():
    ...
    print("\n💾 SINGLE FILE SYSTEM:")
    print("   • Folder: /home/ubuntu/013_2025_polymarket/bot018_Lastsec/BS_data/")
    print("   • File: last_option_data.json (overwritten every second)")
    print("   • Only stores the most recent data point")
