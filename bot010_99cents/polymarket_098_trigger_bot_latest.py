        ...
        # self.wallet_address = self._get_wallet_address()#!/usr/bin/env python3
"""
Polymarket 0.98 Trigger Bot - FIXED VERSION
...
class Polymarket098TriggerBot:
    ...
    def __init__(self, config_path: str = None):
        ...
        # Purchase tracking
        self.purchases_file = '/home/ubuntu/013_2025_polymarket/purchased_tokens.json'

        logger.info("🎯 Polymarket 0.98 Trigger Bot initialized (WEB3 VERSION)")
    ...
    def save_purchase_to_file(self, token_id: str, token_name: str, amount: float, buy_price: float):
        """Save purchased token info to JSON file (append, don't overwrite)"""
        try:
            purchase_data = {
                'token_id': token_id,
            ...
            logger.info(f"💾 Saved purchase to file: {token_name} - {amount} tokens @ ${buy_price:.4f}")

        except Exception as e:
            logger.error(f"❌ Error saving purchase to file: {e}")

    def _load_config(self, config_path: str) -> Dict:
        """Load configuration"""
        default_config = {
            'trigger_bid_price': 0.98,      # Trigger when bid reaches this price
            'buy_price': 0.99,              # Price to buy at when triggered
            ...
            'min_usdc_balance': 100.0,      # Minimum USDC.e balance required (reduced to $100)
    ...
    async def run(self):
        """Main bot loop"""
        logger.info("🎯 Starting 0.98 Trigger Bot (FIXED VERSION)")
...
if __name__ == "__main__":
    print("🎯 POLYMARKET 0.98 TRIGGER BOT - WEB3 VERSION")
    print("=" * 60)
    print("STRATEGY:")
    ...
    print("• ✅ Purchase tracking to JSON file")
    print()
    print("TOKEN TYPES:")
    ...
    print(f"• Min USDC.e balance: $100")
    print(f"• Balance check: Every 60 seconds + before each trade")
    print(f"• Credentials: keys/keys_ovh39.env")
    ...
    print("💡  TIP: Run cleaner.py separately to sell tokens at $1.00")
    print()
