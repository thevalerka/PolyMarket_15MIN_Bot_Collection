...
# Trading parameters
SENS_MULTIPLIER = 1.2
ACTION_THRESHOLD_BALANCED = 0.025  # Threshold when positions are balanced
ACTION_THRESHOLD_IMBALANCED = 0.012  # Lower threshold when imbalanced (easier to rebalance)
...
class Bot016Third:
    ...
    def __init__(self, credentials: dict):
        ...
        self.last_buy_type: Optional[str] = None  # 'CALL' or 'PUT'
        self.last_buy_price: Optional[float] = None
        self.last_pnl_print_time = 0.0  # For 10-second PNL updates
    ...
    def execute_buy(self, token_type: str, token_id: str, ask_price: float,
        ...
        try:
            ...
            if position_confirmed and final_balance >= 0.5:
                ...
                # Record last buy for dynamic threshold logic
                self.last_buy_type = token_type
                self.last_buy_price = ask_price
        ...
        try:
            while True:
                ...
                if period_start != self.current_period_start:
                    ...
                    # Reset last buy tracking for new period
                    self.last_buy_type = None
                ...
                if not in_buffer_zone and not trading_suspended and self.strike_price and prev_btc_price is not None and self.period_asset_ids_validated:
                    ...
                    try:
                        ...
                        # Dynamic ACTION_THRESHOLD based on position balance AND last buy PNL
                        put_count = sum(1 for p in self.positions if p.token_type == 'PUT')
                        call_count = sum(1 for p in self.positions if p.token_type == 'CALL')
                        is_imbalanced = abs(put_count - call_count) > 0

                        # Calculate last buy PNL if we have a last buy and it's still relevant
                        last_buy_pnl = 0.0
                        last_buy_abs_pnl = 0.0
                        can_use_imbalanced_threshold = False

                        if is_imbalanced and self.last_buy_price is not None and self.last_buy_type is not None:
                            # Get current price for the last buy type
                            if self.last_buy_type == 'CALL':
                                current_price = call_bid_price  # Use bid (what we could sell for)
                            else:  # PUT
                                current_price = put_bid_price

                            if current_price > 0:
                                last_buy_pnl = current_price - self.last_buy_price
                                last_buy_abs_pnl = abs(last_buy_pnl)

                                # Can use imbalanced threshold if PNL magnitude > $0.10
                                can_use_imbalanced_threshold = last_buy_abs_pnl > 0.10

                        # Determine threshold
                        if is_imbalanced and can_use_imbalanced_threshold:
                            ACTION_THRESHOLD = ACTION_THRESHOLD_IMBALANCED
                            threshold_reason = f"Imbalanced+PNL(${last_buy_pnl:+.3f})"
                        elif is_imbalanced:
                            ACTION_THRESHOLD = ACTION_THRESHOLD_BALANCED
                            threshold_reason = f"Imbalanced but PNL too small(${last_buy_pnl:+.3f} < $0.10)"
                        else:
                            ACTION_THRESHOLD = ACTION_THRESHOLD_BALANCED
                            threshold_reason = "Balanced"

                        # Print PNL every 10 seconds
                        if time.time() - self.last_pnl_print_time >= 10:
                            if self.last_buy_price is not None and self.last_buy_type is not None:
                                logger.info(f"💰 Last Buy PNL: {self.last_buy_type} @ ${self.last_buy_price:.3f} → Current ${current_price:.3f} = ${last_buy_pnl:+.3f} | Threshold: {ACTION_THRESHOLD} ({threshold_reason})")
                            self.last_pnl_print_time = time.time()
