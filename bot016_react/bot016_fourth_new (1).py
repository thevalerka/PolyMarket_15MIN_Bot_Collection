...
pm2 start polymarket_btc_ws.py --interpreter python3
"""
...
STOP_LOSS = 0.08  # Stop if price moves 0.08 against us
TAKE_PROFIT = 0.16  # Take profit if price moves 0.16 in our favor
...
class Bot016Third:
    ...
    def execute_sell(self, token_type: str, token_id: str, bid_price: float, btc_price: float,
        ...
        """Execute sell order - sells specified quantity or POSITION_SIZE"""
        try:
            ...
            if balance_after < actual_balance - (sell_quantity * 0.5):  # Sold at least half of expected
            ...
            while True:
                ...
                if call_spread > MAX_SPREAD or put_spread > MAX_SPREAD:
                    ...
                    if call_spread > MAX_SPREAD:
                        logger.warning(f"⚠️  TRADING SUSPENDED: CALL spread {call_spread:.3f} > {MAX_SPREAD}")
                    if put_spread > MAX_SPREAD:
                        logger.warning(f"⚠️  TRADING SUSPENDED: PUT spread {put_spread:.3f} > {MAX_SPREAD}")
                ...
                if not in_buffer_zone and len(self.positions) > 0:
                    for position in self.positions[:]:  # Copy list to allow modification
                        ...
                        if current_price > 0:
                            ...
                            # Check stop loss - price moved against us by STOP_LOSS
                            if price_movement <= -STOP_LOSS:
                                logger.info(f"🛑 STOP LOSS: {position.token_type} price moved ${price_movement:.3f} (entry ${position.entry_price:.3f} → ${current_price:.3f})")
