...
STOP_LOSS = 0.09  # Stop if price moves 0.06 against us
...
class Bot016Third:
    ...
    def execute_sell(self, token_type: str, token_id: str, bid_price: float, btc_price: float,
        ...
        try:
            ...
            # Set sell price - AGGRESSIVE for stop loss
            if "STOP_LOSS" in reason:
                # Very aggressive pricing for stop loss - drop price significantly
                sell_price = max(0.01, bid_price - 0.02) if bid_price > 0 else 0.01
                logger.info(f"💰 Sell Price (AGGRESSIVE SL): ${sell_price:.4f}")
            else:
                # Normal pricing for other orders
