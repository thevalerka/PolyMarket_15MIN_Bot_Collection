...
class OscillationStats:
    ...
    max_price: float
    price_range: float
    # Distance traveled (sum of absolute movements)
    call_bid_distance: float  # Total "kilometers" traveled by CALL bid
    put_bid_distance: float   # Total "kilometers" traveled by PUT bid
...
class OscillationAnalyzer:
    ...
    def calculate_distance_traveled(self, prices: List[float]) -> float:
        """
        Calculate total distance traveled (sum of absolute movements)
        Example: 0.50 -> 0.52 -> 0.48 = |0.02| + |0.04| = 0.06
        """
        if len(prices) < 2:
            return 0.0
    ...
    def analyze_csv(self, csv_file: Path) -> OscillationStats:
                ...
                try:
                    ...
                    if idx == 0:
                        if self.is_valid_price(call_bid, call_ask):
                            ...
                            call_bid_prices.append(call_bid)
                            put_bid_prices.append(put_bid)
        ...
        # Calculate distance traveled for CALL and PUT bid prices
        call_bid_distance = self.calculate_distance_traveled(call_bid_prices)
        put_bid_distance = self.calculate_distance_traveled(put_bid_prices)
        ...
        return OscillationStats(
            ...
            call_bid_distance=call_bid_distance,
            put_bid_distance=put_bid_distance,
    ...
    def print_results(self):
        ...
        # Print per-period oscillation stats including distance traveled
        print(f"{'Period':<25} {'Rows':<8} {'$0.05':<6} {'$0.10':<6} {'$0.15':<6} {'Range':<10} {'CALL km':<12} {'PUT km':<12}")
        print(f"{'-'*140}")
        ...
        for stats in self.results:
            print(f"{stats.period_name:<25} {stats.valid_rows:<8} "
                  ...
                  f"${stats.price_range:<9.4f} ${stats.call_bid_distance:<11.4f} ${stats.put_bid_distance:<11.4f}")
        ...
        avg_call_km = statistics.mean([s.call_bid_distance for s in self.results])
        avg_put_km = statistics.mean([s.put_bid_distance for s in self.results])
        
        print(f"{'AVERAGE':<25} {avg_rows:<8.1f} "
              f"{avg_005:<6.2f} {avg_010:<6.2f} {avg_015:<6.2f} "
              f"${avg_range:<9.4f} ${avg_call_km:<11.4f} ${avg_put_km:<11.4f}")
        
        # Calculate totals
        total_call_km = sum([s.call_bid_distance for s in self.results])
        total_put_km = sum([s.put_bid_distance for s in self.results])
        
        print(f"{'TOTAL':<25} {sum([s.valid_rows for s in self.results]):<8} "
              f"{'N/A':<6} {'N/A':<6} {'N/A':<6} "
              f"{'N/A':<10} ${total_call_km:<11.4f} ${total_put_km:<11.4f}")
        ...
        print(f"      CALL bid distance: ${avg_call_km:.4f}")
        print(f"      PUT bid distance: ${avg_put_km:.4f}")
        print(f"      Difference: ${abs(avg_call_km - avg_put_km):.4f}")
        print(f"\n   Total across all periods:")
        print(f"      CALL bid distance: ${total_call_km:.4f}")
        print(f"      PUT bid distance: ${total_put_km:.4f}")
        print(f"      Difference: ${abs(total_call_km - total_put_km):.4f}")
