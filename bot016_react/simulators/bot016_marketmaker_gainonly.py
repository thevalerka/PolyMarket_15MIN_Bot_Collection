#!/usr/bin/env python3
"""
Bot016_MarketMaker_GainOnly - Binary Options Market Making Strategy (NO LOSS)
Provides liquidity by placing limit orders on both sides of the book
Buy at bid - $0.01, Sell ONLY at prices > entry price (guaranteed profit)
NO STOP LOSS - positions are held until profitable
Max 12 shares per side
...
TRADES_DIR = "/home/ubuntu/013_2025_polymarket/bot016_react/mm_trades_gainonly"
...
class MarketMakerBotGainOnly:
    """Market maker for 15-minute BTC binary options - GAIN ONLY (no losses)"""
    ...
    def print_startup_info(self):
        ...
        print("=" * 70)
        print("BOT016_MARKETMAKER_GAINONLY - Never Sell at Loss")
        print(f"Buy: bid-${BUY_OFFSET:.2f} | Sell: ONLY when price > entry + ${MIN_PROFIT_MARGIN:.2f}")
        print(f"Max: {MAX_SHARES_PER_SIDE}/side | NO STOP LOSS")
        print("=" * 70)

    def get_today_filename(self) -> Path:
        """Get filename for today's trades"""
        today = date.today().strftime('%Y%m%d')
        return self.trades_dir / f"mm_gainonly_trades_{today}.json"
    ...
    def emergency_sell_excess(self, token_type: str, token_id: str, current_shares: float, bid_price: float):
        """Emergency sell excess shares when over max - sells at market regardless of profit"""
        excess = current_shares - MAX_SHARES_PER_SIDE
        
        # Minimum sell is 5 shares - sell if excess >= 5
        if excess < 5:
            print(f"⚠️ {token_type} over max by {excess:.0f} but < 5 minimum sell, waiting...")
            return
        
        # Round down to whole shares, but at least 5
        sell_qty = max(5, int(excess))
    ...
    def update_market_making_orders(self, call_bid: float, call_ask: float, put_bid: float, put_ask: float, btc_delta: float = 0):
        """Update market making orders - GAIN ONLY logic"""
        try:
            ...
            # === EMERGENCY SELL EXCESS (when 5+ shares over max) ===
            if call_shares >= MAX_SHARES_PER_SIDE + 5:  # 11+ when max is 6
                self.emergency_sell_excess('CALL', self.current_call_id, call_shares, call_bid)
                return  # Exit and re-evaluate next cycle
            
            if put_shares >= MAX_SHARES_PER_SIDE + 5:  # 11+ when max is 6
                self.emergency_sell_excess('PUT', self.current_put_id, put_shares, put_bid)
                return  # Exit and re-evaluate next cycle

            # === CALL ORDERS ===
            # Buy order - minimum 5 shares required
            ...
            if call_shares >= 5:
                ...
                sell_quantity = max(5, min(int(call_shares), 6))  # At least 5, up to 6
            ...
            if put_shares >= 5:
                ...
                sell_price = round(sell_price, 2)
                sell_quantity = max(5, min(int(put_shares), 6))  # At least 5, up to 6
    ...
    def run(self):
        ...
        logger.info("\n🚀 Starting GAIN-ONLY market maker bot...")
...
def main():
    ...
    try:
        ...
        bot = MarketMakerBotGainOnly(credentials)
        bot.run()
