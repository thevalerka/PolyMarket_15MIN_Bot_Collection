...
class StrategySimulator:
    ...
    def __init__(self, call_file: str, put_file: str, output_dir: str = "/home/ubuntu/013_2025_polymarket/bot016_react/simulators/simulation_results"):
        ...
        self.strategies = {
            ...
            'strategy4': {
                ...
                'description': 'TP/SL at 1/3 distance from $1.00'
            }
        }
    ...
    def should_check_expiry(self, expiry: datetime) -> bool:
        """Check if we're at the expiry check time (14:55 before expiry)"""
        check_time = self.get_expiry_check_time(expiry)
        now = datetime.now()

        # Check if we're within 1 second of the check time
    ...
    def should_skip_entry(self, ask_price: float) -> bool:
        """Check if we should skip entry at this price (e.g., @0.99)"""
        # Never buy at 0.99 or higher
        if ask_price >= 0.99:
            return True
        return False
    ...
    def open_position(self, option_type: str, ask_price: float, strategy_name: str):
        ...
        if self.should_skip_entry(ask_price):
            print(f"[{strategy_name}] Skipping {option_type} entry @ ${ask_price:.4f} (too high)")
            return
    ...
    def check_expiry(self, option_type: str, current_bid: float, strategy_name: str):
        ...
        if self.should_check_expiry(expiry):
            ...
            print(f"[{strategy_name}] ⏰ EXPIRY CHECK for {option_type} | Current Bid: ${current_bid:.4f} | Expiry Price: ${expiry_price:.2f}")
    ...
    def run(self, check_interval: float = 0.5, status_interval: int = 30):
        ...
        print(f"⏰ Expiry evaluation occurs 5 seconds before expiry")
        ...
        print(f"🚫 Never buying at $0.99 or higher\n")
