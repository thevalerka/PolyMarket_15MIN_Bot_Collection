...
BUY_OFFSET = 0.00  # Buy at bid - $0.01
SELL_OFFSET = 0.00  # Sell at ask + $0.01
MAX_SHARES_PER_SIDE = 6  # Maximum 12 shares per side
STOP_LOSS = 0.99  # Stop loss4. at -$0.20/share for imbalanced positions
MIN_SPREAD = 0.03  # Minimum spread required to place orders
BUFFER_SECONDS = 30  # No trading in first 30s of period
POSITION_CHECK_INTERVAL = 3  # Check positions every 30s
...
class MarketMakerBot:
    ...
    def print_startup_info(self):
        ...
        print("="*60)
        print("BOT016_MARKETMAKER - Silent Mode")
        print(f"Buy: bid-${BUY_OFFSET:.2f} | Sell: ask+${SELL_OFFSET:.2f} | Max: {MAX_SHARES_PER_SIDE}/side")
        print("="*60)
    ...
    def place_sell_order(self, token_type: str, token_id: str, price: float, quantity: float) -> Optional[str]:
        ...
        try:
            ...
            if order_id:
                ...
                print(f"SELL {token_type:4} | {quantity:.0f} @ ${price:.2f}")
                return order_id
    ...
    def check_signal_for_reposition(self, btc_price: float, btc_delta: float,
        ...
        try:
            ...
            #print(ideal_call_movement," - ", ideal_put_movement)
    ...
    def update_market_making_orders(self, call_bid: float, call_ask: float, put_bid: float, put_ask: float, btc_delta: float = 0):
        ...
        try:
            ...
            # Print positions
            print(f"\n[POS] CALL={call_shares:.0f}/{MAX_SHARES_PER_SIDE} PUT={put_shares:.0f}/{MAX_SHARES_PER_SIDE} | BTC_Δ=${btc_delta:+.2f}")
