...
TAKE_PROFIT = 0.06  # Take profit at +$0.06/share
STOP_LOSS = 0.09  # Stop loss at -$0.09/share
...
class Bot016Third:
    ...
    def execute_sell_market(self, token_type: str, token_id: str, bid_price: float,
                     btc_price: float, bin_key: str, reason: str) -> bool:
        """Execute sell order using FAK - sells entire balance for token type"""
        try:
            # Find all positions for this token type
            ...
            # Place FAK sell order
            start_time = time.time()
            order_id = self.trader.place_sell_order_market(
                token_id=token_id,
                price=sell_price,
                quantity=actual_balance
            )
                    ...
                    for position in self.positions[:]:  # Copy list to allow modification
                        ...
                        if current_price > 0:
                            ...
                            if pnl_per_share >= TAKE_PROFIT:
                                ...
                                self.execute_sell_market(position.token_type, position.token_id, current_price,
