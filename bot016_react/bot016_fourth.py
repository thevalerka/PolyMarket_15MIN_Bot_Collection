...
class Bot016Third:
    ...
    def verify_position_from_wallet(self):
        """Verify position matches wallet (battle-tested)"""
        ...
        if has_put and not has_call:
            if self.position is None or self.position.token_type != 'PUT':
                ...
                if not put_data.get('best_bid') or not put_data.get('best_ask'): 
                    print("DEEP TIM/OTM : exit")
                    return
        ...
        # Case 2: CALL position
            ...
            if self.position is None or self.position.token_type != 'CALL':
                ...
                if not call_data.get('best_bid') or not call_data.get('best_ask'):
                    print("DEEP TIM/OTM : exit")
                    return
    ...
    def execute_sell(self, token_id: str, bid_price: float, btc_price: float,
        ...
        """Execute sell order (battle-tested)"""
        try:
                ...
                if period_start != self.current_period_start:
                    ...
                    if self.position is not None and self.current_period_start is not None:
                        ...
                        if exit_price and exit_price > 0 and exit_btc > 0:
                            ...
                            self.execute_sell(self.position.token_id, exit_price, exit_btc, bin_key, "PERIOD_END")
                        else:
                        ...
                        if abs(call_sens) > 0.000001 and call_ask_price > 0 and call_bid_price > 0:
                            ...
                            if ideal_call_movement < ACTION_THRESHOLD*-1:
                                ...
                                if self.position and self.position.token_type == 'CALL':
                                    self.execute_sell(self.position.token_id, call_bid_price,
                        ...
                        if abs(put_sens) > 0.000001 and put_ask_price > 0 and put_bid_price > 0:
                            ...
                            if ideal_put_movement > ACTION_THRESHOLD:
                                ...
                                if put_ask_price > 0.95 or put_ask_price < 0.10:
                                    logger.debug(f"❌ PUT BUY rejected: price {call_ask_price:.2f} outside range [0.03, 0.97]")
                                elif self.can_open_position():
                                    edge = ideal_call_movement
                                    self.execute_buy('PUT', self.current_call_id, put_ask_price,
                                                   btc_price, bin_key, edge, f"Edge: {edge:.3f}")
                            ...
                            if ideal_put_movement < ACTION_THRESHOLD*-1:
                                ...
                                if self.position and self.position.token_type == 'PUT':
                                    self.execute_sell(self.position.token_id, put_bid_price,
                                                    btc_price, bin_key, "SELL_SIGNAL")
            ...
            if self.position:
                ...
                if data and data.get('best_bid'):
                    ...
                    if exit_price > 0:
                        ...
                        self.execute_sell(self.position.token_id, exit_price, btc_price, bin_key, "MANUAL_STOP")
