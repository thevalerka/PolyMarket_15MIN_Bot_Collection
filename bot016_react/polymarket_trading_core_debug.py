...
from Valerka_clob_client.client import ClobClient
from py_clob_client.clob_types import ApiCreds, BalanceAllowanceParams, AssetType, OrderArgs, OrderType
...
class PolymarketTrader:
    ...
    def place_buy_order_limit(self, token_id: str, price: float, quantity: float) -> Optional[str]:
        """
        Place a BUY order
        ...
        try:
            ...
            response = self.client.post_order(signed_order,post_only=True)
    ...
    def place_buy_order_FAK(self, token_id: str, price: float, quantity: float) -> Optional[str]:
        """
        Place a BUY order
    ...
    def place_sell_order_FAK(self, token_id: str, price: float, quantity: float) -> Optional[str]:
        """
        Place a SELL order
    ...
    def place_sell_order_limit(self, token_id: str, price: float, quantity: float) -> Optional[str]:
        """
        Place a SELL order
    ...
    def get_price(self, token_id: str, side: str) -> Dict:
        """
        Get price for a specific side (BUY or SELL)
        ...
        Returns:
            Dict with 'price' key containing the requested price
        """
        try:
            # Call the client's get_price method with side parameter
            response = self.client.get_price(token_id, side)
            return response
        
        except Exception as e:
            print(f"❌ Error getting price for {token_id[:8]}... side={side}: {e}")
            raise
