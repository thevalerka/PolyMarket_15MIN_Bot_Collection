#!/usr/bin/env python3
"""
Bot016_DryRun_BuyOnly - Paper Trading Mode (Buy Only)
Only buys CALL/PUT, holds until expiry
Balances PUT/CALL positions (max difference of 1)
Calculates PNL based on binary option expiry (0 or 1)
...
TRADES_DIR = "/home/ubuntu/013_2025_polymarket/bot016_react/dryrun_buyonly_trades"
...
class Position:
    ...
    entry_bin: str
    edge: float
    strike_price: float  # Strike price of the period when bought
    period_start: int  # Period start minute (0, 15, 30, 45)
    signal_reason: str  # What caused the buy (e.g., 'Edge: 0.043' or 'HEDGE_PUT')
...
class Bot016DryRunBuyOnly:
    """Paper trading bot - BUY ONLY mode, holds until expiry"""

    def __init__(self):
        # Position tracking - hold until expiry
        ...
        logger.info("="*80)
        logger.info("🤖 BOT016_DRYRUN_BUYONLY - BUY & HOLD MODE")
        logger.info("="*80)
        logger.info("⚠️  BUY ONLY: Holds until binary expiry (0 or 1)")
        logger.info("📊 Balances PUT/CALL positions (max diff: 1)")
    ...
    def get_today_filename(self) -> Path:
        ...
        return self.trades_dir / f"buyonly_trades_{today}.json"
    ...
    def execute_buy(self, token_type: str, token_id: str, ask_price: float,
        ...
        """SIMULATED buy order - BUY ONLY, hold until expiry"""
        try:
            logger.info(f"\n{'='*70}")
            logger.info(f"📄 PAPER TRADE - BUY & HOLD")
            logger.info(f"{'='*70}")
            ...
            logger.info(f"\n✅ PAPER TRADE SUCCESS: Position opened (hold until expiry)")
            ...
            logger.info(f"📊 Position: {POSITION_SIZE} @ ${ask_price:.4f} | Strike: ${strike_price:.2f}")
            logger.info(f"💰 Paper Balance: ${self.paper_balance:.2f}")
            logger.info(f"📦 Positions: {self.get_position_balance_status()}")
            return True
    ...
    def can_open_position(self, token_type: str) -> bool:
        """Check if we can open a new position - keeps PUT/CALL balanced"""
    ...
    def get_position_balance_status(self) -> str:
        """Get status of PUT/CALL balance"""
        put_count = sum(1 for p in self.positions if p.token_type == 'PUT')
        call_count = sum(1 for p in self.positions if p.token_type == 'CALL')
        diff = abs(put_count - call_count)
        return f"PUT:{put_count} CALL:{call_count} (diff:{diff})"
    ...
    def settle_expired_positions(self, current_period_start: int):
        ...
        for pos in expired_positions:
            ...
            trade = {
                ...
                'close_bin': 'EXPIRY',  # At expiry, no specific bin
        ...
        logger.info(f"Remaining: {self.get_position_balance_status()}")
        logger.info(f"{'='*60}\n")
    ...
    def run(self):
        ...
        try:
            ...
            # Settle all remaining positions
            ...
            logger.info(f"📦 Unsettled positions: {self.get_position_balance_status()}")


def main():
    """Main entry point"""
    try:
        logger.info("🚀 Starting Bot016 DRY RUN - BUY ONLY MODE")
        logger.info("⚠️  HOLDS UNTIL BINARY EXPIRY")

        # Create and run bot (no credentials needed for dry-run)
        bot = Bot016DryRunBuyOnly()
        bot.run()
