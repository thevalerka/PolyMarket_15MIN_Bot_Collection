...
TRADES_DIR = "/home/ubuntu/013_2025_polymarket/bot016_react/seventh_c38_trades"

# Credentials
CREDENTIALS_ENV = '/home/ubuntu/013_2025_polymarket/keys/keys_ovh38.env'
...
TAKE_PROFIT = 0.05  # Take profit at +$0.06/share (LIMIT order)
STOP_LOSS = 0.08  # Stop loss at -$0.09/share (MARKET order)
...
class Bot016Third:
    ...
    def execute_buy(self, token_type: str, token_id: str, ask_price: float,
                    btc_price: float, bin_key: str, edge: float, reason: str, bid_price: float = None) -> bool:
        ...
        try:
            ...
            if position_confirmed and final_balance >= 0.5:
                ...
                self.update_tp_order(token_type, current_bid=bid_price)
    ...
    def update_tp_order(self, token_type: str, current_bid: float = None):
        ...
        try:
            ...
            tp_price = avg_entry + TAKE_PROFIT
            ...
            if current_bid and tp_price <= current_bid:
                # TP already hit - place order at current ask (best_ask = bid + tick)
                # This ensures immediate fill at market price
                tp_price = min(current_bid + 0.01, 0.99)
                logger.warning(f"⚠️  {token_type} TP price (${avg_entry + TAKE_PROFIT:.2f}) would cross book (bid ${current_bid:.2f})")
                logger.info(f"   → Adjusting to best ask: ${tp_price:.2f}")
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # STOP LOSS CHECKS ONLY (TP handled by limit orders)
                if len(self.positions) > 0 and not in_buffer_zone:
                    ...
                    for position in self.positions[:]:  # Copy list to allow modification
                        ...
                        if current_price > 0:
                            ...
                            # STOP LOSS ONLY: <= -$0.09/share (TP handled by limit orders)
                            ...
                            if ideal_call_movement > ACTION_THRESHOLD:
                                ...
                                if call_ask_price > 0.95:
                                    ...
                                    self.execute_buy('CALL', self.current_call_id, call_ask_price,
                                                   btc_price, bin_key, edge, f"Edge: {edge:.3f}", bid_price=call_bid_price)
                            ...
                            if ideal_put_movement > ACTION_THRESHOLD:
                                ...
                                if put_ask_price > 0.95:
                                    ...
                                    self.execute_buy('PUT', self.current_put_id, put_ask_price,
                                                   btc_price, bin_key, edge, f"Edge: {edge:.3f}", bid_price=put_bid_price)
