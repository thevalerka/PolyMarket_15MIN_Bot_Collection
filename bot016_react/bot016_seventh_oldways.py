#!/usr/bin/env python3
"""
Bot016_Seventh - Binary Options with Stop Loss & Take Profit (Coinbase Data)
Uses Coinbase BTC price feed and 15-minute candles for strike price
Take Profit: $0.06/share - sells at bid price (FAK)
Stop Loss: $0.09/share - sells at bid-$0.01 (aggressive FAK)
Buys 6 shares, sells entire balance
...
TRADES_DIR = "/home/ubuntu/013_2025_polymarket/bot016_react/seventh_trades"
...
# Trading parameters
SENS_MULTIPLIER = 1.2
ACTION_THRESHOLD = 0.022  # Single threshold
TAKE_PROFIT = 0.12  # Take profit at +$0.06/share
STOP_LOSS = 0.12  # Stop loss at -$0.09/share
...
MAX_SHARES_PER_SIDE = 12  # Maximum 12 shares per side (CALL or PUT)
...
class Bot016Third:
    ...
    def __init__(self, credentials: dict):
        ...
        logger.info("="*80)
        logger.info("🤖 BOT016_SEVENTH - STOP LOSS & TAKE PROFIT")
        logger.info("="*80)
        logger.info("📡 Data Source: Coinbase BTC-USD")
        logger.info("💰 Take Profit: +$0.06/share (sell at bid)")
        logger.info("🛑 Stop Loss: -$0.09/share (sell at bid-$0.01)")
        logger.info(f"Position Size: {POSITION_SIZE} shares (sell all balance)")
    ...
    def execute_sell(self, token_type: str, token_id: str, bid_price: float,
                     btc_price: float, bin_key: str, reason: str) -> bool:
        ...
        try:
            while True:
                ...
                if btc_data and call_data and put_data:
                    ...
                    if btc_call_diff < -1 :
                        print ("LAG BTC websocket ......",btc_call_diff,"..........  WAIT")
                        time.sleep(0.2)
                        continue

                    if btc_call_diff > 2 :
                        print ("LAG BTC POLYmarket .........",btc_call_diff,".......  WAIT")
                        time.sleep(0.2)
                        continue
                ...
                if not all([btc_data, call_data, put_data]):
                    ...
                    whichdata2 = ""
                    whichdata3 = ""
                    if not btc_data : whichdata = "Missing BTC price"
                    if not call_data : whichdata2 = "Missing CALL price"
                    if not put_data : whichdata2 = "Missing PUT price"
                    print ("LMissing data", whichdata,whichdata2,whichdata3)
                    time.sleep(0.1)
                    continue
                ...
                # STOP LOSS & TAKE PROFIT CHECKS (always active, even in buffer)
                if len(self.positions) > 0 and not in_buffer_zone:
                    ...
                    for position in self.positions[:]:  # Copy list to allow modification
                        ...
                        if current_price > 0:
                            ...
                            # TAKE PROFIT: >= +$0.06/share
                            if pnl_per_share >= TAKE_PROFIT:
                                logger.info(f"💰 TAKE PROFIT: {position.token_type} ${pnl_per_share:+.3f}/share (entry ${position.entry_price:.3f} → ${current_price:.3f})")
                                self.execute_sell(position.token_type, position.token_id, current_price,
                                                btc_price, bin_key, f"TAKE_PROFIT_+${pnl_per_share:.3f}")

                            # STOP LOSS: <= -$0.09/share
                            elif pnl_per_share <= -STOP_LOSS:
                                logger.info(f"🛑 STOP LOSS: {position.token_type} ${pnl_per_share:+.3f}/share (entry ${position.entry_price:.3f} → ${current_price:.3f})")
                                self.execute_sell(position.token_type, position.token_id, current_price,
                                                btc_price, bin_key, f"STOP_LOSS_{pnl_per_share:.3f}")
                ...
                if not in_buffer_zone and not trading_suspended and self.strike_price and prev_btc_price is not None and self.period_asset_ids_validated:
                    ...
                    # Check for signals from sensitivity data
