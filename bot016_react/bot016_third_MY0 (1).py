...
class Bot016Third:
    ...
    def __init__(self, credentials: dict):
        ...
        logger.info("="*80)
        logger.info("🤖 BOT016_THIRD - PRODUCTION MODE")
        logger.info("="*80)
    ...
    def execute_buy(self, token_type: str, token_id: str, ask_price: float,
        ...
        """Execute buy order (battle-tested)"""
        try:
            logger.info(f"\n{'='*70}")
            ...
            if initial_usdc < MIN_BALANCE:
                logger.error(f"❌ INSUFFICIENT BALANCE: ${initial_usdc:.2f} < ${MIN_BALANCE:.2f}")
                return False
            ...
            try:
                ...
                if 'does not exist' in error_str or 'orderbook' in error_str:
                    logger.error(f"❌ Token ID stale, reloading...")
                        ...
                        if abs(call_sens) > 0.000001 and call_ask_price > 0 and call_bid_price > 0:
                            ...
                            if abs(actual_call_ask_movement) <= abs(ideal_call_movement) + 0.02:
                                ...
                                # BUY signal
                                    ...
                                    if call_ask_price > 0.97 or call_ask_price < 0.03:
                        ...
                        if abs(put_sens) > 0.000001 and put_ask_price > 0 and put_bid_price > 0:
                            ...
                            if abs(actual_put_ask_movement) <= abs(ideal_put_movement) + 0.02:
                                ...
                                # BUY signal
                                    ...
                                    # Price constraint: do not buy if price > 0.97 or < 0.03
                                    if put_ask_price > 0.97 or put_ask_price < 0.03:
