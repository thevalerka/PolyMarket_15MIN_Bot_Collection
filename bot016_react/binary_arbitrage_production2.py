...
class BinaryArbitrageProduction:
    ...
    def check_entry_signal(self, snapshot: PriceSnapshot, 
        ...
        if expected_put is None and expected_call is None:
            logger.debug(f"❌ Entry rejected: Both expected prices are None")
        ...
        if expected_put is None and expected_call is None:
            logger.debug(f"❌ Entry rejected: Both None after extreme price filter")
            return None
        ...
        logger.debug(f"Entry check: PUT lag={put_lag:.3f}, CALL lag={call_lag:.3f}, threshold={self.discrepancy_threshold:.3f}")
        
        # Simple lag-based entry (removed spike detection)
        if expected_put and put_lag >= self.discrepancy_threshold:
            logger.info(f"🟢 Entry signal: PUT lag ${put_lag:.3f} >= ${self.discrepancy_threshold:.3f}")
            return ('PUT', snapshot.put_ask, 
                   f"PUT lag ${put_lag:.3f}")
        
        if expected_call and call_lag >= self.discrepancy_threshold:
            logger.info(f"🟢 Entry signal: CALL lag ${call_lag:.3f} >= ${self.discrepancy_threshold:.3f}")
