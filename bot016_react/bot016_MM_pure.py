#!/usr/bin/env python3
"""
Bot016_MM - Binary Options Market Making Strategy
Provides liquidity by placing limit orders on both sides of the book
- Min profit check only for first 15 seconds after buy
- After 15 seconds, sells at any price (no min profit requirement)
- No emergency sell - normal sells only
...
MIN_PROFIT_MARGIN = 0.05  # Minimum profit per share to sell ($0.01)
...
class MarketMakerBotGainSafe:
    """Market maker for 15-minute BTC binary options - min profit check only for first 15s after buy"""
    ...
    def print_startup_info(self):
        ...
        print("=" * 70)
        print("BOT016_MM - Min Profit Check Only First 15 Seconds")
        print(f"Buy: bid-${BUY_OFFSET:.2f} | Sell: entry + ${MIN_PROFIT_MARGIN:.2f} for first 15s, then any price")
        print(f"Max: {MAX_SHARES_PER_SIDE}/side | Normal sell only (no emergency sell)")
        print("=" * 70)
    ...
    def get_earliest_entry_time(self, token_type: str) -> float:
        """Get earliest entry time for a token type (for min_sell_price check)"""
        positions = [p for p in self.positions if p.token_type == token_type]
        if not positions:
            return 0.0  # No position, will always pass the 15s check
    ...
    def place_sell_order(self, token_type: str, token_id: str, price: float, quantity: float, entry_price: float, entry_time: float) -> Optional[str]:
        """Place limit sell order - min profit check only for first 15 seconds after buy"""
        try:
            # GAIN-ONLY CHECK: Only check min_sell_price for first 15 seconds after buy
            seconds_since_buy = time.time() - entry_time

            if seconds_since_buy <= 60:
                # Within first 15 seconds: enforce minimum profit margin
                min_sell_price = entry_price + MIN_PROFIT_MARGIN
                if price < min_sell_price:
                    print(f"⏳ HOLD {token_type:4} | sell ${price:.2f} < entry ${entry_price:.2f} + ${MIN_PROFIT_MARGIN:.2f} = ${min_sell_price:.2f} ({seconds_since_buy:.0f}s since buy)")
                    return None
            # After 15 seconds: sell at any price (no min_sell_price check)
    ...
    def update_market_making_orders(self, call_bid: float, call_ask: float, put_bid: float, put_ask: float, btc_delta: float = 0):
        """Update market making orders - min profit check only for first 15 seconds after buy"""
        try:
            # Calculate position counts and entry prices/times
            call_positions = [p for p in self.positions if p.token_type == 'CALL']
            put_positions = [p for p in self.positions if p.token_type == 'PUT']
            ...
            # Sell order - min profit check only for first 15 seconds after buy
            if call_shares >= 5:
