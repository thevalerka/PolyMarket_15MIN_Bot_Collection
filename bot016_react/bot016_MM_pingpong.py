#!/usr/bin/env python3
"""
Bot016_MM_PingPong - Binary Options Market Making Strategy (PING-PONG)
Alternates between CALL and PUT positions:
- Opens one side first at period start
- Opens the other side ONLY if entry_price + other_ask <= $1.00
- Even positions: just try to sell both
- Odd positions: try to sell + open other side if sum <= $1.00
- No positions: try to open both
...
TRADES_DIR = "/home/ubuntu/013_2025_polymarket/bot016_react/mm_trades_pingpong"
...
MAX_COMBINED_COST = 1.00  # Maximum sum of entry + other side ask
...
class MarketMakerBotPingPong:
    """Market maker for 15-minute BTC binary options - PING-PONG strategy"""
    ...
    def __init__(self, credentials: dict):
        ...
        # PING-PONG: Track which side to open first
        self.first_side_this_period: Optional[str] = None  # 'CALL' or 'PUT'
        self.period_first_open_attempted = False
    ...
    def print_startup_info(self):
        ...
        print("=" * 70)
        print("BOT016_MM_PINGPONG - Ping-Pong Market Making Strategy")
        print(f"Buy: bid-${BUY_OFFSET:.2f} | Sell: ask + ${SELL_OFFSET:.2f} + ${MIN_PROFIT_MARGIN:.2f} margin")
        print(f"Max combined cost: ${MAX_COMBINED_COST:.2f} (entry + other_ask <= $1.00)")
        print(f"Max: {MAX_SHARES_PER_SIDE}/side | Min order: 5 shares")
        print("=" * 70)

    def get_today_filename(self) -> Path:
        """Get filename for today's trades"""
        today = date.today().strftime('%Y%m%d')
        return self.trades_dir / f"mm_pingpong_trades_{today}.json"
    ...
    def get_position_count(self, token_type: str) -> float:
        """Get total shares for a token type"""
        return sum(p.quantity for p in self.positions if p.token_type == token_type)

    def has_position(self, token_type: str) -> bool:
        """Check if we have any position in this token type"""
        return self.get_position_count(token_type) >= 1
    ...
    def decide_first_side(self, call_bid: float, put_bid: float) -> str:
        """Decide which side to open first - pick the cheaper one"""
        # Open the side with lower ask (cheaper to buy)
        # This maximizes chance the other side will also fit under $1.00
        if call_bid <= put_bid:
            return 'CALL'
        else:
            return 'PUT'

    def can_open_other_side(self, existing_entry: float, other_ask: float) -> bool:
        """Check if we can open the other side: entry + other_ask <= $1.00"""
        combined = existing_entry + other_ask
        can_open = combined <= MAX_COMBINED_COST
        if not can_open:
            print(f"   ❌ Cannot open other side: ${existing_entry:.2f} + ${other_ask:.2f} = ${combined:.2f} > ${MAX_COMBINED_COST:.2f}")
        else:
            print(f"   ✅ Can open other side: ${existing_entry:.2f} + ${other_ask:.2f} = ${combined:.2f} <= ${MAX_COMBINED_COST:.2f}")
        return can_open

    def update_pingpong_orders(self, call_bid: float, call_ask: float, put_bid: float, put_ask: float, btc_delta: float = 0):
        """
        PING-PONG order logic:
        - 0 positions: try to open both sides (if combined <= $1.00)
        - 1 position (odd): sell existing + try to open other side if sum <= $1.00
        - 2 positions (even): just try to sell both
        """
        try:
            # Get current positions
            call_shares = self.get_position_count('CALL')
            put_shares = self.get_position_count('PUT')
            call_entry = self.get_average_entry_price('CALL')
            put_entry = self.get_average_entry_price('PUT')

            has_call = call_shares >= 1
            has_put = put_shares >= 1
            total_sides = (1 if has_call else 0) + (1 if has_put else 0)

            print(f"\n[PINGPONG] CALL={call_shares:.0f} (entry ${call_entry:.2f}) | PUT={put_shares:.0f} (entry ${put_entry:.2f}) | Sides={total_sides}")

            # === CASE 0: NO POSITIONS - Try to open both sides ===
            if total_sides == 0:
                print("   📊 State: NO POSITIONS - trying to open both sides")
                
                # Check if we can afford both
                combined_cost = call_bid + put_bid  # Using bids as buy prices
                if combined_cost <= MAX_COMBINED_COST:
                    print(f"   ✅ Combined cost ${call_bid:.2f} + ${put_bid:.2f} = ${combined_cost:.2f} <= ${MAX_COMBINED_COST:.2f}")
                    
                    # Open CALL
                    buy_price_call = max(0.01, call_bid - BUY_OFFSET)
                    buy_price_call = round(buy_price_call, 2)
                    if 0.01 <= buy_price_call <= 0.99:
                        self.place_buy_order('CALL', self.current_call_id, buy_price_call, 5)

                    # Open PUT
                    buy_price_put = max(0.01, put_bid - BUY_OFFSET)
                    buy_price_put = round(buy_price_put, 2)
                    if 0.01 <= buy_price_put <= 0.99:
                        self.place_buy_order('PUT', self.current_put_id, buy_price_put, 5)
                else:
                    # Can't afford both - open the cheaper one first
                    print(f"   ⚠️ Combined cost ${combined_cost:.2f} > ${MAX_COMBINED_COST:.2f} - opening cheaper side only")
                    first_side = self.decide_first_side(call_bid, put_bid)
            ...
            # === CASE 1: ONE POSITION (ODD) - Sell existing + try to open other side ===
            elif total_sides == 1:
                if has_call and not has_put:
                    print("   📊 State: CALL only - selling CALL + trying to open PUT")
                    ...
                    if call_shares >= 5:
                        ...
                        if actual_balance >= 5 and 0.01 <= sell_price <= 0.99:
                            sell_qty = min(int(actual_balance), 6)
                            self.place_sell_order('CALL', self.current_call_id, sell_price, sell_qty, call_entry)
                    
                    # Try to open PUT if sum <= $1.00
                    # Use our CALL entry price + PUT ask price
                    if self.can_open_other_side(call_entry, put_ask):
                        buy_price = max(0.01, put_bid - BUY_OFFSET)
                        buy_price = round(buy_price, 2)
                        if 0.01 <= buy_price <= 0.99:
                            self.place_buy_order('PUT', self.current_put_id, buy_price, 5)
                    ...
                    print("   📊 State: PUT only - selling PUT + trying to open CALL")
                    ...
                    if put_shares >= 5:
                        ...
                        if actual_balance >= 5 and 0.01 <= sell_price <= 0.99:
                            sell_qty = min(int(actual_balance), 6)
                            self.place_sell_order('PUT', self.current_put_id, sell_price, sell_qty, put_entry)
                    
                    # Try to open CALL if sum <= $1.00
                    # Use our PUT entry price + CALL ask price
                    if self.can_open_other_side(put_entry, call_ask):
                        buy_price = max(0.01, call_bid - BUY_OFFSET)
                        buy_price = round(buy_price, 2)
                        if 0.01 <= buy_price <= 0.99:
                            self.place_buy_order('CALL', self.current_call_id, buy_price, 5)

            # === CASE 2: TWO POSITIONS (EVEN) - Just try to sell both ===
            elif total_sides == 2:
                print("   📊 State: BOTH positions - trying to sell both")
            ...
            logger.error(f"❌ Error updating pingpong orders: {e}")
            import traceback
            traceback.print_exc()
    ...
    def run(self):
        ...
        logger.info("\n🚀 Starting PING-PONG market maker bot...")
        ...
        while True:
            try:
                ...
                if period_start != self.current_period_start:
                    ...
                    self.first_side_this_period = None
                    self.period_first_open_attempted = False
                ...
                # WALLET CHECK (only after buffer)
                ...
                if can_place_orders and self.current_call_id and self.current_put_id:
                    ...
                    self.update_pingpong_orders(call_bid, call_ask, put_bid, put_ask, btc_delta_display)
...
def main():
    ...
    try:
        ...
        bot = MarketMakerBotPingPong(credentials)
        bot.run()
