...
class RealTimeLagArbitrageDetector:
    ...
    def detect_arbitrage_opportunity(self, btc_price, call_bid, call_ask, put_bid, put_ask,
        ...
        """Detect arbitrage opportunities using momentum and overreaction analysis - CONSERVATIVE VERSION"""
        opportunities = []
        ...
        # Only look for opportunities when there are SIGNIFICANT movements

        # MOMENTUM-BASED ARBITRAGE DETECTION
        if len(self.btc_prices) >= 5:  # Require more data points for stability
            # Get recent price movements (longer window)
            recent_btc = list(self.btc_prices)[-5:]
            recent_call_bids = list(self.call_bids)[-5:]
            recent_call_asks = list(self.call_asks)[-5:]
            recent_put_bids = list(self.put_bids)[-5:]
            recent_put_asks = list(self.put_asks)[-5:]

            # Calculate BTC movement over longer period
            btc_change = recent_btc[-1] - recent_btc[0]  # 5-point change
            btc_pct_change = btc_change / recent_btc[0]

            # Only analyze SIGNIFICANT BTC moves
            min_btc_move = 50  # Minimum $50 move
            min_pct_move = 0.0005  # Minimum 0.05% move

            if abs(btc_change) > min_btc_move and abs(btc_pct_change) > min_pct_move:
                ...
                # Conservative expected movement calculation
                # Expect roughly 5-10 cents per $100 BTC move
                expected_option_change = abs(btc_change) * 0.001  # 0.1% of BTC move
                expected_option_change = max(0.03, min(0.15, expected_option_change))  # 3-15 cents range

                # VERY CONSERVATIVE OVERREACTION DETECTION
                overreaction_threshold = 3.0  # Need 3x overreaction (was 1.5x)
                min_profit = 0.05  # Need 5 cent profit minimum (was 1.5 cents)

                if btc_change < 0:  # BTC went down
                    # Check for PUT overreaction only
                    if put_mid_change > expected_option_change * overreaction_threshold:
                        profit_potential = (put_mid_change - expected_option_change) * 0.5  # Conservative 50%
                ...
                elif btc_change > 0:  # BTC went up
                    # Check for CALL overreaction only
                    if call_mid_change > expected_option_change * overreaction_threshold:
                        profit_potential = (call_mid_change - expected_option_change) * 0.5
                        if profit_potential > min_profit:
        ...
        # EXTREME MISPRICING DETECTION ONLY
        # Only look for very obvious mispricings

        # Deep in-the-money options priced too low
        if moneyness > 1.05 and call_ask < 0.50:  # BTC 5%+ above strike, call under 50%
            profit_potential = (0.80 - call_ask) * 0.6  # Conservative estimate
            if profit_potential > 0.10:  # 10 cent minimum
                opp = self._create_opportunity('CALL', 'BUY', call_ask, btc_price, strike_price,
                                             moneyness, time_to_expiry, profit_potential,
                                             f'Deep ITM call severely underpriced: ${call_ask:.3f} (moneyness {moneyness:.3f})')
                opportunities.append(opp)

        elif moneyness < 0.95 and put_ask < 0.50:  # BTC 5%+ below strike, put under 50%
            profit_potential = (0.80 - put_ask) * 0.6
            if profit_potential > 0.10:
                opp = self._create_opportunity('PUT', 'BUY', put_ask, btc_price, strike_price,
                                             moneyness, time_to_expiry, profit_potential,
                                             f'Deep ITM put severely underpriced: ${put_ask:.3f} (moneyness {moneyness:.3f})')
                opportunities.append(opp)

        # Far out-of-the-money options priced too high
        elif moneyness < 0.95 and call_bid > 0.30:  # BTC 5%+ below strike, call bid over 30%
            profit_potential = (call_bid - 0.15) * 0.6
            if profit_potential > 0.10:
                opp = self._create_opportunity('CALL', 'SELL', call_bid, btc_price, strike_price,
                                             moneyness, time_to_expiry, profit_potential,
                                             f'Deep OTM call overpriced: ${call_bid:.3f} (moneyness {moneyness:.3f})')
                opportunities.append(opp)

        elif moneyness > 1.05 and put_bid > 0.30:  # BTC 5%+ above strike, put bid over 30%
            profit_potential = (put_bid - 0.15) * 0.6
            if profit_potential > 0.10:
                opp = self._create_opportunity('PUT', 'SELL', put_bid, btc_price, strike_price,
                                             moneyness, time_to_expiry, profit_potential,
                                             f'Deep OTM put overpriced: ${put_bid:.3f} (moneyness {moneyness:.3f})')
                opportunities.append(opp)
    ...
    def process_data_point(self, btc_data, call_data, put_data):
        ...
        # Calculate moneyness and distance from strike (moved here to avoid UnboundLocalError)
