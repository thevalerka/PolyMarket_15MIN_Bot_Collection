...
def create_ml_trading_config():
    ...
    config = MLTradingConfig(
        ...
        # Conservative trading parameters for initial testing
        max_position_size_usd=5,       # Start with $10 positions
        max_total_exposure_usd=10,      # $50 total exposure limit
        min_confidence_threshold=0.70,    # 70% ML confidence required
        min_profit_threshold=0.02,        # 2 cent minimum profit
        max_trades_per_hour=45,            # Conservative rate limiting

        # Risk management (aligned with your existing setup)
        stop_loss_threshold=-0.23,        # 3 cent stop loss
        take_profit_threshold=0.08,       # 8 cent take profit
        max_hold_time_minutes=20,         # 20 minute max hold

        # Order execution
        order_timeout_seconds=60,
        price_buffer_cents=0.001,         # 0.1 cent buffer
        min_order_size=1.0,
    )
